...
@AutoValue
public abstract class SpannerRowsCheck extends ConditionCheck {

  abstract SpannerResourceManager resourceManager();
  ...
  public String getDescription() {
    if (maxRows() != null) {
      return String.format(
          "Spanner check if table %s has between %d and %d rows", tableId(), minRows(), maxRows());
    }
    return String.format("Spanner check if table %s has %d rows", tableId(), minRows());
  }
  ...
  public static Builder builder(SpannerResourceManager resourceManager, String tableId) {
    return new AutoValue_SpannerRowsCheck.Builder()
        .setResourceManager(resourceManager)
        .setTableId(tableId);
  }

  /** Builder for {@link SpannerRowsCheck}. */
  @AutoValue.Builder
  public abstract static class Builder {

    public abstract Builder setResourceManager(SpannerResourceManager resourceManager);
    ...
    abstract SpannerRowsCheck autoBuild();

    public SpannerRowsCheck build() {
      return autoBuild();
    }
  }
}
