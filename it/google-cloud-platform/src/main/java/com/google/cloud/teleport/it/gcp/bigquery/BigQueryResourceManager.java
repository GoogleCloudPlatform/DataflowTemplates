...
public final class BigQueryResourceManager implements ResourceManager {
   ...
   * @throws BigQueryResourceManagerException if there is an exeption during query execution.
   */
   ...
   *
   * @param tableName The name of the table to insert the given rows into.
   * @throws BigQueryResourceManagerException if there is an exception when attempting to get the
   *     row count.
   */
  public Long getRowCount(String tableName) {
    TableResult r =
        runQuery(String.format("SELECT COUNT(*) FROM `%s.%s.%s`", projectId, datasetId, tableName));
