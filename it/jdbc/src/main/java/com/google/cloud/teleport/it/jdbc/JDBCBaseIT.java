...
public abstract class JDBCBaseIT extends TemplateTestBase {
  ...
  // The system property name for the JDBC driver versions.
  // NOTE: These property names must correspond to the values declared in the `it/jdbc/pom.xml`
  // file.
  private static final String MYSQL_VERSION_PROPERTY = "mySqlVersion";
  private static final String POSTGRES_VERSION_PROPERTY = "postgresVersion";
  private static final String ORACLE_VERSION_PROPERTY = "oracleVersion";
  private static final String MSSQL_VERSION_PROPERTY = "msSqlVersion";
  ...
  private static String mySqlDriverLocalJar() {
    return String.join("-", MYSQL_JAR_PREFIX, sqlVersion(MYSQL_VERSION_PROPERTY)) + JAR_SUFFIX;
  }

  private static String postgresDriverLocalJar() {
    return String.join("-", POSTGRES_JAR_PREFIX, sqlVersion(POSTGRES_VERSION_PROPERTY))
        + JAR_SUFFIX;
  }

  private static String oracleDriverLocalJar() {
    return String.join("-", ORACLE_JAR_PREFIX, sqlVersion(ORACLE_VERSION_PROPERTY)) + JAR_SUFFIX;
  }

  private static String msSqlDriverLocalJar() {
    return String.join("-", MSSQL_JAR_PREFIX, sqlVersion(MSSQL_VERSION_PROPERTY)) + JAR_SUFFIX;
  }

  private static String mySqlDriverLocalPath(String basePath) {
    return String.join(
        "/", basePath, MYSQL_LOCAL_PATH, sqlVersion(MYSQL_VERSION_PROPERTY), mySqlDriverLocalJar());
  }
  ...
  private static String postgresDriverLocalPath(String basePath) {
    return String.join(
        ...
        basePath,
        POSTGRES_LOCAL_PATH,
        sqlVersion(POSTGRES_VERSION_PROPERTY),
        postgresDriverLocalJar());
  }
  ...
  private static String oracleDriverLocalPath(String basePath) {
    return String.join(
        ...
        basePath,
        ORACLE_LOCAL_PATH,
        sqlVersion(ORACLE_VERSION_PROPERTY),
        oracleDriverLocalJar());
  }

  private static String msSqlDriverLocalPath(String basePath) {
    return String.join(
        "/", basePath, MSSQL_LOCAL_PATH, sqlVersion(MSSQL_VERSION_PROPERTY), msSqlDriverLocalJar());
  }

  private static String sqlVersion(String property) {
    if (!Strings.isNullOrEmpty(System.getProperty(property))) {
      return System.getProperty(property);
    }
    throw new RuntimeException("Missing system property for -D" + property);
  }
