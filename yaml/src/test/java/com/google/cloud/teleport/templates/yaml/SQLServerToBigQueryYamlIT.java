...
/** Integration test for {@link SQLServerToBigQueryYaml} template. */
@Category({TemplateIntegrationTest.class, SkipDirectRunnerTest.class})
@TemplateIntegrationTest(SQLServerToBigQueryYaml.class)
@RunWith(JUnit4.class)
public class SQLServerToBigQueryYamlIT extends TemplateTestBase {
  ...
  @Test
  public void testSQLServerToBigQuery() throws IOException {
    // 1. Setup SQL Server database and insert test data
    JDBCResourceManager.JDBCSchema jdbcSchema =
        new JDBCResourceManager.JDBCSchema(Map.of("id", "INTEGER", "name", "VARCHAR(100)"), "id");

    msSqlResourceManager.createTable(TABLE_NAME, jdbcSchema);
    msSqlResourceManager.write(
        TABLE_NAME,
