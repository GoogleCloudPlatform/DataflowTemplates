...
* **jdbcUrl**: The JDBC connection URL. For example, `jdbc:oracle:thin:@//your-host:1521/your-service`.
...
* **driverClassName**: The fully-qualified class name of the JDBC driver to use. For example, `oracle.jdbc.OracleDriver`. Defaults to: oracle.jdbc.OracleDriver.
* **driverJars**: A comma-separated list of GCS paths to the JDBC driver JAR files. For example, `gs://your-bucket/ojdbc8.jar`.
...
* **connectionInitSql**: A list of SQL statements to execute when a new connection is established. For example, `["ALTER SESSION SET CURRENT_SCHEMA=my_schema"]`.
