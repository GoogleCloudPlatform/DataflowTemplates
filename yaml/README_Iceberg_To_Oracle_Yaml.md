...
Iceberg to Oracle (YAML) template
---
The Iceberg to Oracle template is a batch pipeline that reads data from an
Iceberg table and outputs the records to a Oracle database table.
...
```
Flex Template was staged! gs://<bucket-name>/templates/flex/Iceberg_To_Oracle_Yaml
```
...
export TEMPLATE_SPEC_GCSPATH="gs://$BUCKET_NAME/templates/flex/Iceberg_To_Oracle_Yaml"
...
resource "google_dataflow_flex_template_job" "iceberg_to_oracle_yaml" {

  provider          = google-beta
  container_spec_gcs_path = "gs://dataflow-templates-${var.region}/latest/flex/Iceberg_To_Oracle_Yaml"
  name              = "iceberg-to-oracle-yaml"
  region            = var.region
  parameters        = {
    table = "<table>"
