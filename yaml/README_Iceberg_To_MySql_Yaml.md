...
Iceberg to MySql (YAML) template
---
The Iceberg to MySql template is a batch pipeline that executes the user provided
SQL query to read data from Iceberg and outputs the records to MySQL table.
...
```
Flex Template was staged! gs://<bucket-name>/templates/flex/Iceberg_To_MySql_Yaml
```
...
export TEMPLATE_SPEC_GCSPATH="gs://$BUCKET_NAME/templates/flex/Iceberg_To_MySql_Yaml"
...
```shell
cd v2/yaml/terraform/Iceberg_To_MySql_Yaml
terraform init
terraform apply
```
...
resource "google_dataflow_flex_template_job" "iceberg_to_mysql_yaml" {

  provider          = google-beta
  container_spec_gcs_path = "gs://dataflow-templates-${var.region}/latest/flex/Iceberg_To_MySql_Yaml"
  name              = "iceberg-to-mysql-yaml"
  region            = var.region
  parameters        = {
    jdbcUrl = "<jdbcUrl>"
    location = "<location>"
