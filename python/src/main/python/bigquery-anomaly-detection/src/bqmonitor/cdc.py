...
class _ExecuteQueryFn(beam.DoFn):
  ...
  def _get_or_create_temp_dataset(self) -> None:
    ...
    Sets a default table expiration so orphaned temp tables (e.g. from
    pipeline crashes before cleanup) are automatically garbage-collected.
    """
    try:
      ...
      if e.status_code != 404:
        raise
      _LOGGER.info(
          '[Query] Creating temp dataset %s:%s (location=%s)',
          self._project, self._temp_dataset, self._location)
      dataset_ref = bigquery.DatasetReference(
          projectId=self._project, datasetId=self._temp_dataset)
      dataset = bigquery.Dataset(
          datasetReference=dataset_ref,
          defaultTableExpirationMs=_DEFAULT_TABLE_EXPIRATION_MS)
