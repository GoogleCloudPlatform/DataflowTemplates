...
public final class BigQueryAnomalyDetectionIT extends TemplateTestBase {
  ...
  private void testSimpleSumMetric() throws IOException, InterruptedException {
    ...
    LaunchConfig.Builder options =
        LaunchConfig.builder(testName, specPath)
            ...
            .addParameter("topic", outputTopic.getTopic())
  ...
  /**
   * Tests the Threshold detector with a fixed expression (value >= 100).
   *
   * <p>Inserts rows with amount=10 (below threshold), then a batch with amount=500 (above
   * threshold). No warmup period is needed — the threshold fires immediately on any value that
