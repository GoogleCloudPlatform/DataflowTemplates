...
public abstract class JavascriptTextTransformer {
  ...
  public abstract static class JavascriptRuntime {
    ...
    public String invoke(String data) throws ScriptException, IOException, NoSuchMethodException {
      ...
      if (result == null || ScriptRuntime.UNDEFINED == result) {
        return null;
    ...
    /**
     * Loads into memory scripts from a File System from a given path. Supports any file system that {@link FileSystems} supports.
     *
  ...
  /**
   * The {@link FailsafeJavascriptUdf} class processes user-defined functions is a fail-safe manner by maintaining the original payload
   * post-transformation and outputting to a dead-letter on failure.
   */
  @AutoValue
  ...
  /**
   * Retrieves a {@link JavascriptRuntime} configured to invoke the specified function within the script. If either the fileSystemPath or
   * functionName is null or empty, this method will return null indicating that a runtime was unable to be created within the given
   * parameters.
   *
