  ...
  public static PipelineResult run(Options options) {
    ...
    if (options.getUseSubscription()) {
      messages =
        ...
        Read<PubsubMessage> xms = PubsubIO.readMessagesWithAttributes().fromTopic(options.getInputTopic());
        Read<PubsubMessage> tableName = xms.withIdAttribute("hpi_tablename");
        Map<TupleTag<?>, PValue> tmp = xms.getAdditionalInputs();
        
        if(tableName != null){
            ValueProvider<String> vS = ValueProvider.StaticValueProvider.of(tableName.getName());
            options.setOutputTableSpec(vS);
        }
      messages =
          pipeline.apply(
              "ReadPubSubTopic",
  ...
  static class PubsubMessageToTableRow
    ...
    public PCollectionTuple expand(PCollection<PubsubMessage> input) {
      ...
      if(!options.getUseSubscription()){
        ...
        Read<PubsubMessage> xms = PubsubIO.readMessagesWithAttributes().fromTopic(options.getInputTopic());
        PCollection<String> cx = input.apply("Get TableName In PubsubMessage", ParDo.of(new DoFn<PubsubMessage, String>()
        {
          @ProcessElement
          public void processElement(ProcessContext c)
          {
            String tablenNameTmp = c.element().getAttribute("hpi_tablename");
            c.output(tablenNameTmp);
          }
        }));

        Map<TupleTag<?>, PValue> ex = cx.expand();
        if(ex != null){
          for(Entry<TupleTag<?>, PValue> tag : ex.entrySet()){
            String tableName = tag.getValue().getName();
            if(tableName != null){
              ValueProvider<String> vS = ValueProvider.StaticValueProvider.of(tableName);
              options.setOutputTableSpec(vS);
              break;
            }
          }
