...
```shell
cd <#if flex><#if spec.metadata.module! == "yaml" || spec.metadata.module! == "python">${spec.metadata.module!}<#else>v2/${spec.metadata.module!}</#if>/terraform/${spec.metadata.internalName}<#else>v1/terraform/${spec.metadata.internalName}</#if>
terraform init
terraform apply
```
