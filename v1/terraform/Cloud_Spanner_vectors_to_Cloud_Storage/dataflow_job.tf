...
variable "gcsOutputFolder" {
  type        = string
  description = "The Cloud Storage folder to write output files to. The path must end with a slash. For example, `gs://your-bucket/folder1/`"

}

variable "gcsOutputFilePrefix" {
  type        = string
  description = "The filename prefix for writing output files. For example, `vector-embeddings`"

}

variable "spannerHost" {
  type        = string
  description = "The Spanner endpoint to call in the template. The default value is https://batch-spanner.googleapis.com. For example, `https://batch-spanner.googleapis.com`"
  default     = null
}
