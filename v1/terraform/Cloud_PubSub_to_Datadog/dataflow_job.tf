...
variable "inputSubscription" {
  type        = string
  description = "The Pub/Sub subscription to read the input from. (Example: projects/your-project-id/subscriptions/your-subscription-name)"

}

variable "apiKey" {
  type        = string
  description = "The Datadog API key. You must provide this value if the `apiKeySource` is set to `PLAINTEXT` or `KMS`. For more information, see API and Application Keys (https://docs.datadoghq.com/account_management/api-app-keys/) in the Datadog documentation."
  default     = null
}

variable "url" {
  type        = string
  description = "The Datadog Logs API URL. This URL must be routable from the VPC that the pipeline runs in. See Send logs (https://docs.datadoghq.com/api/latest/logs/#send-logs) in the Datadog documentation for more information. (Example: https://http-intake.logs.datadoghq.com)"

}

variable "batchCount" {
  type        = number
  description = "The batch size for sending multiple events to Datadog. The default is `1` (no batching)."
  default     = null
}

variable "parallelism" {
  type        = number
  description = "The maximum number of parallel requests. The default is `1` (no parallelism)."
  default     = null
}

variable "includePubsubMessage" {
  type        = bool
  description = "Whether to include the full Pub/Sub message in the payload. The default is `true` (all elements, including the data element, are included in the payload)."
  default     = null
}

variable "apiKeyKMSEncryptionKey" {
  type        = string
  description = "The Cloud KMS key to use to decrypt the API Key. You must provide this parameter if the `apiKeySource` is set to `KMS`. If the Cloud KMS key is provided, you must pass in an encrypted API Key. (Example: projects/your-project-id/locations/global/keyRings/your-keyring/cryptoKeys/your-key-name)"
  default     = null
}

variable "apiKeySecretId" {
  type        = string
  description = "The Secret Manager secret ID for the API Key. You must provide this parameter if the `apiKeySource` is set to `SECRET_MANAGER`. (Example: projects/your-project-id/secrets/your-secret/versions/your-secret-version)"
  default     = null
}
