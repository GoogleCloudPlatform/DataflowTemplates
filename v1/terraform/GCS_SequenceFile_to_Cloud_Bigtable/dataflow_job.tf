...
variable "sourcePattern" {
  type        = string
  description = "The Cloud Storage path pattern to the location of the data. For example, `gs://your-bucket/your-path/prefix*`"

}

variable "mutationThrottleLatencyMs" {
  type        = number
  description = "Optional Set mutation latency throttling (enables the feature). Value in milliseconds. Defaults to: 0."
  default     = null
}
...
resource "google_dataflow_job" "generated" {
  ...
  parameters = {
    ...
    sourcePattern             = var.sourcePattern
    mutationThrottleLatencyMs = tostring(var.mutationThrottleLatencyMs)
  }
