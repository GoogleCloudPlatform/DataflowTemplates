...
/**
 * A Beam transform that imports a set of text files to a Cloud Spanner
 * database.
 */
public class TextImportTransform extends PTransform<PBegin, PDone> {
  ...
  public PDone expand(PBegin begin) {
    ...
    PCollectionView<Dialect> dialectView = begin
        .apply("Read Dialect", new ReadDialect(spannerConfig))
        .apply("Dialect As PCollectionView", View.asSingleton());

    PCollection<Ddl> ddl = begin.apply(
        "Read Information Schema", new ReadInformationSchema(spannerConfig, tx, dialectView));

    PCollectionView<Ddl> ddlView = ddl.apply("Cloud Spanner DDL as view", View.asSingleton());

    PCollection<ImportManifest> manifest = begin.apply("Read manifest file",
        new ReadImportManifest(importManifest, dialectView));

    PCollection<KV<String, String>> allFiles = manifest.apply("Resolve data files",
        new ResolveDataFiles(importManifest, ddlView));

    PCollection<Map<String, List<TableManifest.Column>>> tableColumns = manifest
        .apply("Read table columns from manifest", new ReadTableColumns());

    PCollectionView<Map<String, List<TableManifest.Column>>> tableColumnsView = tableColumns
        .apply("tableColumns as View", View.asSingleton());
    ...
    PCollection<HashMultimap<Integer, String>> levelMap = ddl.apply(
    ...
    PCollection<HashMultimap<String, String>> tablesToFilesMap = tableFiles
    ...
    for (int i = 0; i < MAX_DEPTH; i++) {
      final int depth = i;
      PCollection<KV<String, String>> levelFileToTables = tablesToFilesMap.apply(
          "Store depth " + depth,
          ParDo.of(
      ...
      PCollection<Mutation> mutations = levelFileToTables
      ...
      SpannerWriteResult result = mutations
      ...
      result
          ...
          .apply(
              ...
              ParDo.of(
                  new DoFn<com.google.cloud.teleport.spanner.spannerio.MutationGroup, String>() {
                    private final Counter failedSpannerWrites = Metrics.counter(TextImportTransform.class,
                        "FailedSpannerWrites");
          ...
          .apply(
              ...
              TextIO.<String>writeCustomType()
                  ...
                  .withSuffix(java.util.UUID.randomUUID().toString() + "-spanner-depth-" + depth + ".csv")
                  .skipIfEmpty()
  ...
  private class TextTableFilesAsMutations
    ...
    public PCollection<Mutation> expand(PCollection<KV<String, String>> filesToTables) {
      ...
      PCollectionView<Map<String, String>> filesToTablesMapView = filesToTables.apply("asView", View.asMap());
      TextImportPipeline.Options options = filesToTables.getPipeline().getOptions()
          .as(TextImportPipeline.Options.class);
      ...
      PCollectionTuple outputCollections = filesToTables
  ...
  /**
   * Read contents of the import manifest file, which is a json file with the
   * following format: [ {
   * "table": "table_1", "files": [ "table_1_data_file_1", "table_1_data_file_2",
   * "table_1_data_file_3" ] }, { "table": "table_2", "files": [
   * "table_2_data_file*"] } ]. When
   * using GLOB patterns in the files field, please make sure the patterns match
   * input files
   * properly. Take TPC-H benchmark data files as an example, where input files
   * include part.tbl.1,
   * part.tbl.2, partsupp.tbl.1, and partsupp.tbl.2. Table PARTSUPP can use
   * pattern "partsupp*".
   * However, table PART cannot use "part*", as it will incorrectly match file
   * partsupp.tbl.1 as
   * well. Using a more specific pattern such as "part.tbl.*" will solve the
   * issue.
   */
  @VisibleForTesting
  static class ReadImportManifest extends PTransform<PBegin, PCollection<ImportManifest>> {
    ...
    public PCollection<ImportManifest> expand(PBegin input) {
      ...
      PCollection<ImportManifest> manifest = input
  ...
  static class ResolveDataFiles
                    ...
                    for (String pattern : tableManifest.getFilePatternsList()) {
                      try {
                        if (isGcs) {
                          ...
                          if (parent != null) {
                            DirectoryStream<Path> matchingFiles = Files.newDirectoryStream(
    ...
    private static void validateManifest(TableManifest tableManifest, Ddl ddl) {
      ...
      for (TableManifest.Column manifestColumn : tableManifest.getColumnsList()) {
        ...
        if (parseSpannerDataType(manifestColumn.getTypeName(), ddl.dialect()) != dbColumn.type().getCode()) {
          throw new RuntimeException(
              String.format(
  ...
  static class ReadTableColumns
      extends PTransform<PCollection<ImportManifest>, PCollection<Map<String, List<TableManifest.Column>>>> {
          ...
          ParDo.of(
              new DoFn<ImportManifest, Map<String, List<TableManifest.Column>>>() {
                ...
                public void processElement(ProcessContext c) {
                  ...
                  Map<String, List<TableManifest.Column>> columnsMap = new HashMap<String, List<TableManifest.Column>>();
