...
* **firestoreWriteProjectId** : The Google Cloud project ID of where to write Firestore entities.
...
* **firestoreHintNumWorkers** : Hint for the expected number of workers in the Firestore ramp-up throttling step. Defaults to 500 if not specified.
...
-DtemplateName="GCS_Text_to_Firestore" \
-Dparameters="textReadPattern=$TEXT_READ_PATTERN,javascriptTextTransformGcsPath=$JAVASCRIPT_TEXT_TRANSFORM_GCS_PATH,javascriptTextTransformFunctionName=$JAVASCRIPT_TEXT_TRANSFORM_FUNCTION_NAME,firestoreWriteProjectId=$FIRESTORE_WRITE_PROJECT_ID,firestoreHintNumWorkers=$FIRESTORE_HINT_NUM_WORKERS,errorWritePath=$ERROR_WRITE_PATH" \
-f v1
```
...
```shell
cd v1/terraform/GCS_Text_to_Firestore
terraform init
terraform apply
```
