...
variable "bootstrapServers" {
  type        = string
  description = "The host address of the running Apache Kafka broker servers in a comma-separated list. Each host address must be in the format `35.70.252.199:9092`. For example, `localhost:9092,127.0.0.1:9093`"
  default     = null
}

variable "kafkaReadTopics" {
  type        = string
  description = "Kafka topic(s) to read input from. For example, `topic1,topic2`"
  default     = null
}

variable "inputTopics" {
  type        = string
  description = "The Apache Kafka input topics to read from in a comma-separated list.  For example, `topic1,topic2`"
  default     = null
}
