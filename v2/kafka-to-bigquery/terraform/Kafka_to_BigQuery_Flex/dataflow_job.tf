...
variable "schemaFormat" {
  type        = string
  description = "The Kafka schema format. Can be provided as SINGLE_SCHEMA_FILE or SCHEMA_REGISTRY. If SINGLE_SCHEMA_FILE is specified, all messages should have the schema mentioned in the avro schema file. If SCHEMA_REGISTRY is specified, the messages can have either a single schema or multiple schemas. Defaults to: SINGLE_SCHEMA_FILE."
  default     = null
}
...
variable "schemaRegistryTruststoreLocation" {
  type        = string
  description = "Location of the SSL certificate where the trust store for authentication to Schema Registry are stored. (Example: /your-bucket/truststore.jks)"
  default     = null
}

variable "schemaRegistryTruststorePasswordSecretId" {
  type        = string
  description = "SecretId in secret manager where the password to access secret in truststore is stored. (Example: projects/your-project-number/secrets/your-secret-name/versions/your-secret-version)"
  default     = null
}

variable "schemaRegistryKeystoreLocation" {
  type        = string
  description = "Keystore location that contains the SSL certificate and private key. (Example: /your-bucket/keystore.jks)"
  default     = null
}

variable "schemaRegistryKeystorePasswordSecretId" {
  type        = string
  description = "SecretId in secret manager where the password to access the keystore file (Example: projects/your-project-number/secrets/your-secret-name/versions/your-secret-version)"
  default     = null
}

variable "schemaRegistryKeyPasswordSecretId" {
  type        = string
  description = "SecretId of password required to access the client's private key stored within the keystore (Example: projects/your-project-number/secrets/your-secret-name/versions/your-secret-version)"
  default     = null
}
...
variable "schemaRegistryOauthClientSecretId" {
  type        = string
  description = "The Google Cloud Secret Manager secret ID that contains the Client Secret to use to authenticate the Schema Registry client in OAUTH mode. Required for AVRO_CONFLUENT_WIRE_FORMAT message format. (Example: projects/<PROJECT_ID>/secrets/<SECRET_ID>/versions/<SECRET_VERSION>)"
  default     = null
}

variable "schemaRegistryOauthScope" {
  type        = string
  description = "The access token scope used to authenticate the Schema Registry client in OAUTH mode. This field is optional, as the request can be made without a scope parameter passed. (Example: openid)"
  default     = null
}
...
variable "outputDeadletterTable" {
  type        = string
  description = "Fully Qualified BigQuery table name for failed messages. Messages failed to reach the output table for different reasons (e.g., mismatched schema, malformed json) are written to this table.The table will be created by the template. (Example: your-project-id:your-dataset.your-table-name)"
  default     = null
}
