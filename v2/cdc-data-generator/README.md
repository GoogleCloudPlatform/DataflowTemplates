...
4. Schedules N follow-up UPDATEs via a processing-time `Timer`, spaced
   `x s` apart, where N is `updateQps / insertQps` for the table and x is the `update interval`.
5. Optionally schedules a final DELETE after the last UPDATE based on the `delete interval` and `deleteQps / insertQps`.
...
* **`insertQps` / `updateQps` / `deleteQps`** are *targets*. Actual
  ...
  per-shard write capacity) and worker count.
...
* If the worker CPU is pegged but `recordsWritten` is well below the
  sink's headroom, raise `--maxNumWorkers`.
* Increase `--batchSize` to amortise per-batch overhead. Caveat: too
...
* **Logical types**: `STRING`, `INT64`, `FLOAT64`, `NUMERIC`, `BOOLEAN`,
  `BYTES`, `DATE`, `TIMESTAMP`, `JSON` are fully
  supported.
