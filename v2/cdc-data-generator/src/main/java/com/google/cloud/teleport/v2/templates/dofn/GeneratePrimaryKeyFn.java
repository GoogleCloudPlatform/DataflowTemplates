...
public class GeneratePrimaryKeyFn extends DoFn<DataGeneratorTable, KV<String, Row>> {
   ...
   *
   * <p>Unlike the data-generator reference implementation, {@code DataGeneratorColumn} in this
   * module does not carry an {@code isPrimaryKey()} flag, so PK columns are identified by
   * cross-referencing {@link DataGeneratorTable#primaryKeys()} (a list of PK column names)
   * against {@link DataGeneratorTable#columns()}. This preserves PK column order (as declared
   * on the table) regardless of the order columns appear in {@code columns()}.
   */
  @VisibleForTesting
