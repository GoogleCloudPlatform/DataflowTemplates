...
variable "inputSubscription" {
  type        = string
  description = "Name of the Pub/Sub subscription. For example, `projects/your-project-id/subscriptions/your-subscription-name`"

}

variable "mongoDBUri" {
  type        = string
  description = "Comma separated list of MongoDB servers. For example, `host1:port,host2:port,host3:port`"
...
variable "collection" {
  type        = string
  description = "Name of the collection in the MongoDB database. For example, `my-collection`"

}

variable "deadletterTable" {
  type        = string
  description = "The BigQuery table that stores messages caused by failures, such as mismatched schema, malformed JSON, and so on. For example, `your-project-id:your-dataset.your-table-name`"
...
variable "maxConnectionIdleTime" {
  type        = number
  description = "Maximum idle time allowed in seconds before connection timeout occurs. Defaults to: 60000."
  default     = null
}
...
resource "google_dataflow_flex_template_job" "generated" {
  ...
  parameters = {
    ...
    maxConnectionIdleTime                   = tostring(var.maxConnectionIdleTime)
    sslEnabled                              = tostring(var.sslEnabled)
    ignoreSSLCertificate                    = tostring(var.ignoreSSLCertificate)
    withOrdered                             = tostring(var.withOrdered)
    withSSLInvalidHostNameAllowed           = tostring(var.withSSLInvalidHostNameAllowed)
