...
variable "inputSubscription" {
  type        = string
  description = "Pub/Sub subscription to consume the input from. For example, `projects/<PROJECT_ID>/subscriptions/<SUBSCRIPTION_NAME>`"

}

variable "dataset" {
  type        = string
  description = "The type of logs sent using Pub/Sub, for which we have an out-of-the-box dashboard. Known log types values are `audit`, `vpcflow`, and `firewall`. Defaults to: `pubsub`."
  default     = null
}

variable "namespace" {
  type        = string
  description = "An arbitrary grouping, such as an environment (dev, prod, or qa), a team, or a strategic business unit. Defaults to: `default`."
  default     = null
}

variable "errorOutputTopic" {
  type        = string
  description = "The Pub/Sub output topic for publishing failed records, in the format of `projects/<PROJECT_ID>/topics/<TOPIC_NAME>`."
