...
public class SpannerInformationSchemaScannerTest {
  ...
  @Before
  public void setUp() {
    spannerConfig = SpannerConfig.create()
  ...
  public void testConvertDdlToSourceSchema_simpleTable() {
    Ddl ddl = Ddl.builder()
        .createTable("Singers")
          ...
          .column("FirstName").string().max().endColumn()
    ...
    SourceColumn idCol = table.columns().stream().filter(c -> c.name().equals("SingerId")).findFirst().get();
    assertEquals("INT64", idCol.type());
    assertFalse(idCol.isNullable());
    assertTrue(idCol.isPrimaryKey());

    SourceColumn nameCol = table.columns().stream().filter(c -> c.name().equals("FirstName")).findFirst().get();
  ...
  public void testConvertTable_variousTypes() {
    Table spannerTable = Table.builder()
        .name("AllTypes")
        .column("bool_col").bool().endColumn()
        .column("int_col").int64().endColumn()
        .column("float_col").float64().endColumn()
        .column("str_col").string().max().endColumn()
        .column("bytes_col").bytes().max().endColumn()
        .column("date_col").date().endColumn()
        .column("ts_col").timestamp().endColumn()
        .column("num_col").numeric().endColumn()
        .column("json_col").json().endColumn()
        .column("arr_col").type(Type.array(Type.int64())).endColumn()
        .primaryKey().asc("int_col").end()
        .build();
  ...
  public void testConvertTable_generatedColumn() {
    Table spannerTable = Table.builder()
        ...
        .column("gen_col").int64().generatedAs("id * 2").endColumn()
        .primaryKey().asc("id").end()
        .build();

    SourceTable sourceTable = scanner.convertTable(spannerTable);
    
    SourceColumn genCol = sourceTable.columns().stream().filter(c -> c.name().equals("gen_col")).findFirst().get();
    assertTrue(genCol.isGenerated());
    
    SourceColumn idCol = sourceTable.columns().stream().filter(c -> c.name().equals("id")).findFirst().get();
  ...
  public void testSpannerTypeToString() {
    ...
    assertEquals("TIMESTAMP", SpannerInformationSchemaScanner.spannerTypeToString(Type.timestamp()));
    ...
    assertEquals("ARRAY<INT64>", SpannerInformationSchemaScanner.spannerTypeToString(Type.array(Type.int64())));
    assertEquals("ARRAY<STRING>", SpannerInformationSchemaScanner.spannerTypeToString(Type.array(Type.string())));
    assertEquals("ARRAY<ARRAY<INT64>>", SpannerInformationSchemaScanner.spannerTypeToString(Type.array(Type.array(Type.int64()))));
  }
}
