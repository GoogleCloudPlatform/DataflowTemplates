...
public class ChangeEventTypeConvertor {
  ...
  public static Value toValue(
    ...
    switch (columnType.getCode()) {
      case BOOL:
      case PG_BOOL:
        return Value.bool(ChangeEventTypeConvertor.toBoolean(changeEvent, key, requiredField));
      case INT64:
      case PG_INT8:
        return Value.int64(ChangeEventTypeConvertor.toLong(changeEvent, key, requiredField));
      case FLOAT64:
      case PG_FLOAT8:
        return Value.float64(ChangeEventTypeConvertor.toDouble(changeEvent, key, requiredField));
      case FLOAT32:
      case PG_FLOAT4:
        return Value.float32(ChangeEventTypeConvertor.toFloat(changeEvent, key, requiredField));
      case STRING:
      case PG_VARCHAR:
      case PG_TEXT:
        return Value.string(ChangeEventTypeConvertor.toString(changeEvent, key, requiredField));
      case NUMERIC:
      case PG_NUMERIC:
        return Value.numeric(
            ChangeEventTypeConvertor.toNumericBigDecimal(changeEvent, key, requiredField));
      case JSON:
      case PG_JSONB:
        return Value.string(ChangeEventTypeConvertor.toString(changeEvent, key, requiredField));
      case BYTES:
      case PG_BYTEA:
        return Value.bytes(ChangeEventTypeConvertor.toByteArray(changeEvent, key, requiredField));
      case TIMESTAMP:
      case PG_COMMIT_TIMESTAMP:
      case PG_TIMESTAMPTZ:
        return Value.timestamp(
            ChangeEventTypeConvertor.toTimestamp(changeEvent, key, requiredField));
      case DATE:
      case PG_DATE:
        return Value.date(ChangeEventTypeConvertor.toDate(changeEvent, key, requiredField));
      case ARRAY:
