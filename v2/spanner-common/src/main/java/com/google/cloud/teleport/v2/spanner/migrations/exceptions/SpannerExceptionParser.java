...
import org.apache.hadoop.yarn.webapp.hamlet2.Hamlet.COL;
...
public class SpannerExceptionParser {
  ...
  public static final String CELL_VALUE_TOO_LARGE = "Invalid commit request: maximum size for a single column value is";
  ...
  public static final String FK_VIOLATION_PARENT_DELETE = "Foreign key constraint violation when deleting or updating referenced row(s)";
  public static final String FK_VIOLATION_PARENT_UPDATE = "Integrity constraint violation during DELETE/REPLACE. Found child row";
  ...
  public static SpannerMigrationException parse(SpannerException exception) {
    ...
    switch (exception.getErrorCode()) {
      ...
      case NOT_FOUND -> {
        ...
        } else if (containsIgnoreCase(exception.getMessage(), PARENT_NOT_FOUND_1) && containsIgnoreCase(exception.getMessage(), PARENT_NOT_FOUND_2)) {
      ...
      case FAILED_PRECONDITION -> {
        ...
        } else if (containsIgnoreCase(exception.getMessage(), GENERATED_PK_NOT_FULLY_SPECIFIED_1)
        ...
        } else if (containsIgnoreCase(exception.getMessage(), FK_VIOLATION_CHILD_INSERT_1) && containsIgnoreCase(exception.getMessage(), FK_VIOLATION_CHILD_INSERT_2)) {
