...
/**
 * This mapper uses an SMT session file to map table and column names. For fetching destination data types, it uses {@link Ddl}.
 */
public class SessionBasedMapper implements ISchemaMapper {
    ...
    static void validateSchemaAndDdl(Schema schema, Ddl ddl) throws InputMismatchException {
        ...
        List<String> ddlTableNames = ddl.allTables().stream().map(table -> table.name()).collect(Collectors.toList());
        Collections.sort(ddlTableNames);
        if (!schemaTableNames.equals(ddlTableNames)) {
            throw new InputMismatchException("List of spanner table names found in session file do not match tables that actually exist on Spanner. Please provide a valid session file.");
        }
        for (String tableName : ddlTableNames) {
            ...
            List<String> ddlColNames = ddl.table(tableName).columns().stream()
                    .map(column -> column.name())
                    .collect(Collectors.toList());
            Collections.sort(ddlColNames);
            if (!schemaColNames.equals(ddlColNames)) {
                throw new InputMismatchException(String.format("List of spanner column names found in session file do not match columns that actually exist on Spanner for table '%s'. Please provide a valid session file.", tableName));
            }
        }
    }
