...
public abstract class MappedObjectCache<KeyT, ValueT> implements Serializable {
  ...
  public MappedObjectCache withCacheResetTimeUnitValue(Integer value) {
    this.cachedObjects = CacheBuilder.newBuilder()
        .expireAfterWrite(value, TimeUnit.MINUTES)
        .<KeyT, ValueT>build();
   ...
   *
   * @param key a key used to lookup the value in the set.
   * @param currentValue is the current ValueT which a thread is using
   * and if the stored value is already different than supply that.
   */
