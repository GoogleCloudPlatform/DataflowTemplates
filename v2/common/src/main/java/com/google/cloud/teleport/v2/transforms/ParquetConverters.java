...
public class ParquetConverters {
  ...
  public interface ParquetOptions extends PipelineOptions {
    ...
    @Description("GCS bucket to write output file(s) to (e.g. gs://mybucket/path/).")
  ...
  public abstract static class ReadParquetFile
    ...
    public PCollection<GenericRecord> expand(PBegin input) {
      return input.apply(
          "ReadParquetFile",
          ParquetIO.read(AvroConverters.getAvroSchema(schema())).from(inputFileSpec()));
    }
   ...
   */
  @AutoValue
  public abstract static class WriteParquetFile extends PTransform<PCollection<GenericRecord>, POutput> {
    ...
    public POutput expand(PCollection<GenericRecord> input) {
      return input.apply(
          ...
          FileIO.<GenericRecord>write()
              .via(ParquetIO.sink(AvroConverters.getAvroSchema(schema())))
              .to(outputFile())
