...
variable "inputSubscriptions" {
  type        = string
  description = "The comma-separated list of Pub/Sub input subscriptions to read from, in the format `<SUBSCRIPTION_NAME>,<SUBSCRIPTION_NAME>, ...`"

}

variable "changeLogDataset" {
  type        = string
  description = "The BigQuery dataset to store the staging tables in, in the format <DATASET_NAME>."

}

variable "replicaDataset" {
  type        = string
  description = "The location of the BigQuery dataset to store the replica tables in, in the format <DATASET_NAME>."

}

variable "updateFrequencySecs" {
  type        = number
  description = "The interval at which the pipeline updates the BigQuery table replicating the MySQL database."
  default     = null
}

variable "useSingleTopic" {
  type        = bool
  description = "Set this to true if you have configured your Debezium connector to publish all table updates to a single topic. Defaults to: false."
  default     = null
}
...
resource "google_dataflow_flex_template_job" "generated" {
  ...
  parameters = {
    inputTopics                           = var.inputTopics
    inputSubscriptions                    = var.inputSubscriptions
    changeLogDataset                      = var.changeLogDataset
    replicaDataset                        = var.replicaDataset
    updateFrequencySecs                   = tostring(var.updateFrequencySecs)
    useSingleTopic                        = tostring(var.useSingleTopic)
