...
variable "inputSubscription" {
  type        = string
  description = "The Pub/Sub input subscription to read from, in the format of 'projects/<PROJECT_ID>/subscriptions/<SUBSCRIPTION_NAME>' (Example: projects/your-project-id/subscriptions/your-subscription-name)"
...
variable "connectionUrl" {
  type        = string
  description = "The JDBC connection URL string. You can pass in this value as a string that's encrypted with a Cloud KMS key and then Base64-encoded. Remove whitespace characters from the Base64-encoded string. (Example: jdbc:mysql://some-host:3306/sampledb)"

}

variable "username" {
  type        = string
  description = "The username to use for the JDBC connection. You can pass in this value as a string that's encrypted with a Cloud KMS key and then Base64-encoded."
  default     = null
}

variable "password" {
  type        = string
  description = "The password to use for the JDBC connection. You can pass in this value as a string that's encrypted with a Cloud KMS key and then Base64-encoded."
  default     = null
}
...
variable "connectionProperties" {
  type        = string
  description = "The properties string to use for the JDBC connection. The string must use the format `[propertyName=property;]*`.  (Example: unicode=true;characterEncoding=UTF-8)"
  default     = null
}

variable "statement" {
  type        = string
  description = "The statement to run against the database. The statement must specify the column names of the table in any order. Only the values of the specified column names are read from the JSON and added to the statement.  (Example: INSERT INTO tableName (column1, column2) VALUES (?,?))"

}

variable "outputDeadletterTopic" {
  type        = string
  description = "The Pub/Sub topic to forward undeliverable messages to.  (Example: projects/<PROJECT_ID>/topics/<TOPIC_NAME>)"
