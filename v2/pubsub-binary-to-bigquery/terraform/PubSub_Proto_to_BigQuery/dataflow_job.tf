...
variable "protoSchemaPath" {
  type = string
  description = "Cloud Storage path to a self-contained descriptor set file. Example: gs://MyBucket/schema.pb. `schema.pb` can be generated by adding `--descriptor_set_out=schema.pb` to the `protoc` command that compiles the protos. The `--include_imports` flag can be used to guarantee that the file is self-contained."
  
}

variable "fullMessageName" {
  type = string
  description = "The full message name (example: package.name.MessageName). If the message is nested inside of another message, then include all messages with the '.' delimiter (example: package.name.OuterMessage.InnerMessage). 'package.name' should be from the `package` statement, not the `java_package` statement."
  
}

variable "preserveProtoFieldNames" {
  type = string
  description = "Flag to control whether proto field names should be kept or converted to lowerCamelCase. If the table already exists, this should be based on what matches the table's schema. Otherwise, it will determine the column names of the created table. True to preserve proto snake_case. False will convert fields to lowerCamelCase. (Default: false)"
  default = null
}

variable "bigQueryTableSchemaPath" {
  type = string
  description = "Cloud Storage path to the BigQuery schema JSON file. If this is not set, then the schema is inferred from the Proto schema. (Example: gs://MyBucket/bq_schema.json)"
  default = null
}

variable "udfOutputTopic" {
  type = string
  description = "An optional output topic to send UDF failures to. If this option is not set, then failures will be written to the same topic as the BigQuery failures. (Example: projects/your-project-id/topics/your-topic-name)"
  default = null
}
...
resource "google_dataflow_flex_template_job" "generated" {
    ...
    parameters = {
        ...
        preserveProtoFieldNames = var.preserveProtoFieldNames
