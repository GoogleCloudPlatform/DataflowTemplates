...
public class CustomTransformationWithShardForBulkIT implements ISpannerMigrationTransformer {
  ...
  public MigrationTransformationResponse toSpannerRow(MigrationTransformationRequest request)
    ...
    if (request.getTableName().equals("Customers")) {
      ...
      responseRow.put("full_name", row.get("first_name") + " " + row.get("last_name"));
      responseRow.put("migration_shard_id", request.getShardId() + "_" + row.get("id"));
      ...
      try {
        ...
        responseRow.put("datetime_column", dateTimeFormat.format(calendar.getTime()));
        dateTime = dateTimeFormat.parse((String) row.get("timestamp_column"));
        calendar.setTime(dateTime);
        calendar.add(Calendar.SECOND, -1);
        responseRow.put("timestamp_column", dateTimeFormat.format(calendar.getTime()));
      ...
      if (row.containsKey("varbinary_column")) {
        responseRow.put("varbinary_column", "0102030405060708090A0B0C0D0E0F1011121314");
      }
      if (row.containsKey("char_column")) {
        responseRow.put("char_column", "newchar");
      }
      if (row.containsKey("longblob_column")) {
        responseRow.put("longblob_column", "576f726d64");
      }
      if (row.containsKey("longtext_column")) {
        responseRow.put("longtext_column", row.get("longtext_column") + " append");
      }
      ...
      if (row.containsKey("tinytext_column")) {
        responseRow.put("tinytext_column", row.get("tinytext_column") + " append");
      }
      if (row.containsKey("json_column")) {
        responseRow.put("json_column", "{\"k1\": \"v1\", \"k2\": \"v2\"}");
      }
      MigrationTransformationResponse response =
          new MigrationTransformationResponse(responseRow, false);
      return response;
