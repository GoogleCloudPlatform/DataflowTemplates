...
1. Source Setup
    - **For MySQL:**
        ...
        - Check that the MySQL credentials are correctly specified in the [source shards file](#sample-source-shards-file-for-mysql-and-postgresql).
        - Check that the MySQL server is up.
        - The MySQL user configured in the [source shards file](#sample-source-shards-file-for-mysql-and-postgresql) should have [INSERT](https://dev.mysql.com/doc/refman/8.0/en/privileges-provided.html#priv_insert), [UPDATE](https://dev.mysql.com/doc/refman/8.0/en/privileges-provided.html#priv_update) and [DELETE](https://dev.mysql.com/doc/refman/8.0/en/privileges-provided.html#priv_delete) privileges on the database.
    - **For PostgreSQL:**
        - Ensure network connectivity between the PostgreSQL database and your GCP project, where your Dataflow jobs will run.
        - Allowlist Dataflow worker IPs on the PostgreSQL instance so that they can access the PostgreSQL IPs.
...
### Sample source shards file for MySQL and PostgreSQL

This file contains meta data regarding the source MySQL or PostgreSQL shards, which is used to connect to them. This should be present even if there is a single source database shard.
...
11. **PostgreSQL:** Replicating array columns to native PostgreSQL array types (e.g., `varchar[]`) is not fully supported and is converted to comma-separated strings instead.
12. **PostgreSQL:** Writing to non-default schemas (namespaces) is not supported; records will be written to the default schema of the connected user.
