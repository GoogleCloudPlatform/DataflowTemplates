  ...
  public void processElement(
    ...
    if (shardId == null || shardId.equals(Constants.SEVERE_ERROR_SHARD_ID)) {
      ...
      try {
        ...
        String stateVal = latestProcessedState.read();
        ShadowTableRecord shadowTableRecord = null;
        if (stateVal == null) {
          shadowTableRecord =
              spannerDao.readShadowTableRecordWithNoLock(shadowTableName, primaryKey);
        } else {
          String[] parts = stateVal.split("\\|");
          shadowTableRecord =
              new ShadowTableRecord(
                  com.google.cloud.Timestamp.parseTimestamp(parts[0]), Long.parseLong(parts[1]));
        }
        ...
        if (!isSourceAhead) {
          ...
          if (Math.random() < 0.1) {
            LOG.warn("Simulating partial failure before shadow table write!");
            throw new RuntimeException("Simulated partial failure for testing!");
          }
