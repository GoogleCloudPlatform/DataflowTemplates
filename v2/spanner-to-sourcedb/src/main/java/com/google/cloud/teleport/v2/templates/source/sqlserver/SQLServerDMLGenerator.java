...
public class SQLServerDMLGenerator implements IDMLGenerator {
  ...
  static String getMappedColumnValue(
    ...
    } else if (colType.getCode().equals(Type.Code.JSON)
        || colType.getCode().equals(Type.Code.PG_JSONB)) {
      if (valuesJson.optJSONObject(colName) != null) {
        colInputValue = valuesJson.getJSONObject(colName).toString();
      } else if (valuesJson.optJSONArray(colName) != null) {
        colInputValue = valuesJson.getJSONArray(colName).toString();
      } else {
        colInputValue = valuesJson.getString(colName);
      }
