...
public final class MySQLDMLGeneratorTest {
  ...
  public void testGeneratedPrimaryKeyDML() {
    String sessionFile = "src/test/resources/generatedColumnSession.json";
    Ddl ddl = SchemaUtils.buildSpannerDdlFromSessionFile(sessionFile);
    // Let's modify the DDL to make FirstName the primary key, and it's a generated
    // column
    // Since we don't have a session file with a generated PK readily available for
    // this test specifically,
    // we can construct one programmatically or mock the behaviour.
    // However, the easiest is to just use DDL builder:
    Ddl modifiedDdl =
        ddl.toBuilder()
