...
public class SpannerToSourceDbRenameColumnIT extends SpannerToSourceDbITBase {

  private static final Logger LOG = LoggerFactory.getLogger(SpannerToSourceDbRenameColumnIT.class);

  private static final String SPANNER_DDL_RESOURCE =
      "SpannerToSourceDbColumnRenameIT/spanner-schema.sql";
  private static final String SESSION_FILE_RESOURCE =
      "SpannerToSourceDbColumnRenameIT/session.json";
  private static final String MYSQL_SCHEMA_FILE_RESOURCE =
      "SpannerToSourceDbColumnRenameIT/mysql-schema.sql";

  private static final String TABLE = "Users";
  private static final HashSet<SpannerToSourceDbRenameColumnIT> testInstances = new HashSet<>();
  ...
  public void setUp() throws IOException {
    ...
    synchronized (SpannerToSourceDbRenameColumnIT.class) {
      testInstances.add(this);
      if (jobInfo == null) {
        spannerResourceManager =
            createSpannerDatabase(SpannerToSourceDbRenameColumnIT.SPANNER_DDL_RESOURCE);
        spannerMetadataResourceManager = createSpannerMetadataDatabase();

        jdbcResourceManager = MySQLResourceManager.builder(testName).build();

        createMySQLSchema(
            jdbcResourceManager, SpannerToSourceDbRenameColumnIT.MYSQL_SCHEMA_FILE_RESOURCE);
  ...
  public static void cleanUp() throws IOException {
    for (SpannerToSourceDbRenameColumnIT instance : testInstances) {
  ...
  @Test
  public void spannerToSourceDbRenameColumn() throws InterruptedException {
  ...
  private void writeRowInSpanner() {
    ...
    Mutation m =
        Mutation.newInsertOrUpdateBuilder("Users").set("id").to(1).set("new_name").to("FF").build();
