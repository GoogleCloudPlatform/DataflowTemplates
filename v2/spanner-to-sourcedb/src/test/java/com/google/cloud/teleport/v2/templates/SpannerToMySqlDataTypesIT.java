...
public class SpannerToMySqlDataTypesIT extends SpannerToSourceDbITBase {
  ...
  public void setUp() throws IOException {
    spannerResourceManager =
        createSpannerDatabase(SpannerToMySqlDataTypesIT.SPANNER_DDL_RESOURCE);
    spannerMetadataResourceManager = createSpannerMetadataDatabase();

    jdbcResourceManager = MySQLResourceManager.builder(testName).build();

    createMySQLSchema(
        jdbcResourceManager, SpannerToMySqlDataTypesIT.MYSQL_SCHEMA_FILE_RESOURCE);
  ...
  private void assertRowInMySQL() {
    ...
    for (Map.Entry<String, List<Map<String, Object>>> expectedTableData : expectedData.entrySet()) {
      ...
      if (tableName.equals("time_table")) {
        // JDBC Time objects represent a wall-clock time and not a duration (as MySQL treats them). Need to read them as a string to avoid a DataReadException
        rawRows = jdbcResourceManager.runSQLQuery("SELECT id, CAST(time_col as char) as time_col FROM time_table");
      } else {
  ...
  // Replaces `null` values with the string "NULL" and byte arrays with the base64 encoding of the bytes
  ...
  private Map<String, List<Map<String, Object>>> getExpectedData() {
    ...
    // Year gets read out of the DB as a java.sql.Date, so it includes the month/day (both defaulted to 1); the actual data in the DB is just the year
