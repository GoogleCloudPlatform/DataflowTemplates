...
/** Integration test for checking timezone conversion with SQL Server. */
@Category({TemplateIntegrationTest.class, SkipDirectRunnerTest.class})
@TemplateIntegrationTest(SpannerToSourceDb.class)
@RunWith(JUnit4.class)
public class SpannerToSQLServerTimezoneIT extends SpannerToSourceDbITBase {

  private static final Logger LOG = LoggerFactory.getLogger(SpannerToSQLServerTimezoneIT.class);

  private static final String SPANNER_DDL_RESOURCE =
      "sqlserver/SpannerToSQLServerTimezoneIT/spanner-schema.sql";
  private static final String SESSION_FILE_RESOURCE =
      "sqlserver/SpannerToSQLServerTimezoneIT/session.json";
  private static final String SQLSERVER_SCHEMA_FILE_RESOURCE =
      "sqlserver/SpannerToSQLServerTimezoneIT/sqlserver-schema.sql";

  private static final String TABLE = "Users";
  private static final HashSet<SpannerToSQLServerTimezoneIT> testInstances = new HashSet<>();
  ...
  public void setUp() throws IOException {
    ...
    synchronized (SpannerToSQLServerTimezoneIT.class) {
      testInstances.add(this);
      if (jobInfo == null) {
        spannerResourceManager =
            createSpannerDatabase(SpannerToSQLServerTimezoneIT.SPANNER_DDL_RESOURCE);
        spannerMetadataResourceManager = createSpannerMetadataDatabase();

        jdbcResourceManager = setUpMSSQLResourceManager(testName);
        createSQLServerSchema(
            jdbcResourceManager, SpannerToSQLServerTimezoneIT.SQLSERVER_SCHEMA_FILE_RESOURCE);

        gcsResourceManager = setUpSpannerITGcsResourceManager();
        createAndUploadShardConfigToGcs(gcsResourceManager, jdbcResourceManager);
        gcsResourceManager.uploadArtifact(
            "input/session.json",
            Resources.getResource(SpannerToSQLServerTimezoneIT.SESSION_FILE_RESOURCE).getPath());
  ...
  public static void cleanUp() throws IOException {
    for (SpannerToSQLServerTimezoneIT instance : testInstances) {
