...
/**
 * Integration test for reverse replication from Spanner to Oracle using the retryAllDLQ mode.
 *
...
public class SpannerToSourceDBOracleRetryAllDLQIT extends SpannerToSourceDbITBase {

  private static final Logger LOG =
      LoggerFactory.getLogger(SpannerToSourceDBOracleRetryAllDLQIT.class);
  private static final String SPANNER_DDL_RESOURCE =
      "oracle/SpannerToSourceDBOracleRetryAllDLQIT/oracle-spanner-schema.sql";
  private static final String ORACLE_SCHEMA_FILE_RESOURCE =
      "oracle/SpannerToSourceDBOracleRetryAllDLQIT/oracle-schema.sql";
  private static final String OVERRIDES_FILE_RESOURCE =
      "oracle/SpannerToSourceDBOracleRetryAllDLQIT/overrides.json";

  private static final HashSet<SpannerToSourceDBOracleRetryAllDLQIT> testInstances =
      new HashSet<>();
  ...
  public void setUp() throws IOException, InterruptedException {
    ...
    synchronized (SpannerToSourceDBOracleRetryAllDLQIT.class) {
      testInstances.add(this);
      if (jobInfo == null) {
        spannerResourceManager =
            createSpannerDatabase(SpannerToSourceDBOracleRetryAllDLQIT.SPANNER_DDL_RESOURCE);
  ...
  public static void cleanUp() throws IOException {
    for (SpannerToSourceDBOracleRetryAllDLQIT instance : testInstances) {
  ...
  public void testSpannerToSrcDBRetryAllDLQ() throws Exception {
    ...
    // 1. Insert parent rows directly into Oracle. This prevents out-of-order Dataflow failures
    ...
    // 8. Verify target Oracle database has the correct updated state.
    ...
    LOG.info("Verified target Oracle database contents successfully");
  }
  ...
  private Map<String, Object> createExpectedRow999() {
    ...
    row.put("binary_col", "bin".getBytes()); // RAW(255) is NOT right-padded with zeros
    row.put("varbinary_col", "varbin".getBytes());
    row.put("bit_col", new byte[] {1}); // Oracle RAW(8) stores exactly what is pushed
    ...
    expectedRow.forEach(
        (key, expectedValue) -> {
          ...
          if (expectedValue == null) {
            ...
            boolean actualBool = ((java.lang.Number) actualValue).intValue() == 1;
            assertTrue(
                "Field " + key + " mismatch: expected " + expectedValue + " but got " + actualValue,
