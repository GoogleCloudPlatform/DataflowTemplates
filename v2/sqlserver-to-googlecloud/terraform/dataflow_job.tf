...
variable "connectionURL" {
  type = string
  description = "The JDBC connection URL string. Can be passed in as a string that's Base64-encoded and then encrypted with a Cloud KMS key. (Example: jdbc:sqlserver://localhost;databaseName=sampledb)"
}
...
variable "useColumnAlias" {
  type = bool
  description = "If enabled (set to true) the pipeline will consider column alias ("AS") instead of the column name to map the rows to BigQuery. Defaults to false."
  default = false
}
