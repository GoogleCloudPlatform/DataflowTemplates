...
variable "connectionURL" {
  type        = string
  description = "The JDBC connection URL string. Can be passed in as a string that's Base64-encoded and then encrypted with a Cloud KMS key, or can be a Secret Manager secret in the form projects/{project}/secrets/{secret}/versions/{secret_version}. (Example: jdbc:sqlserver://localhost;databaseName=sampledb)"
