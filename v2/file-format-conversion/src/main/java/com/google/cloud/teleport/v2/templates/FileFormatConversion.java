 ...
 * Supported file transformations are:
 * <ul>
 *     <li>Csv to Avro</li>
 *     <li>Csv to Parquet</li>
 *     <li>Avro to Parquet</li>
 *     <li>Parquet to Avro</li>
 * </ul>
 *
...
public class FileFormatConversion {
  ...
  /**
   * Utility method to check if a file format exists in {@link ValidFileFormats}.
   *
   * @param fileFormat to be checked.
   * @return True if the file format exists in {@link ValidFileFormats}.
   */
  private static boolean checkFileFormat(String fileFormat) {
    for (ValidFileFormats validFileFormats : ValidFileFormats.values()) {
      if (validFileFormats.name().equals(fileFormat)) {
        return true;
      }
    }
    return false;
  ...
  public static PipelineResult run(FileFormatConversionOptions options) {
    ...
    try {
      ...
      if (!checkFileFormat(inputFileFormat) || !checkFileFormat(outputFileFormat)) {
    ...
    pipeline.apply(
        ...
        FileFormatConversionFactory.FileFormat.newBuilder().setOptions(options).build());
