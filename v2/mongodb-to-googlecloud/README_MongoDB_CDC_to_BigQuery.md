...
on how to use it without having to build from sources using [Create job from template](https://console.cloud.google.com/dataflow/createjob?template=MongoDB_CDC_to_BigQuery).
...
-DtemplateName="MongoDB_CDC_to_BigQuery" \
...
```
Flex Template was staged! gs://<bucket-name>/templates/flex/MongoDB_CDC_to_BigQuery
```
...
export TEMPLATE_SPEC_GCSPATH="gs://$BUCKET_NAME/templates/flex/MongoDB_CDC_to_BigQuery"
...
```shell
cd v2/mongodb-to-googlecloud/terraform/MongoDB_CDC_to_BigQuery
terraform init
terraform apply
```
...
resource "google_dataflow_flex_template_job" "MongoDB_CDC_to_BigQuery" {
