...
public class SourceWriterFn extends DoFn<KV<Long, TrimmedShardedDataChangeRecord>, Void> {
  ...
  public void setup() {
    ...
    for (Shard shard : shards) {
      ...
      mySqlDaoMap.put(
          ...
          new MySqlDao(sourceConnectionUrl, shard.getUserName(), shard.getPassword(), null));
      // mySqlDaoMap.put(shard.getLogicalShardId(), new MySqlDao("dummy"));
    }
    spannerDao = new SpannerDao(spannerConfig, "", false);
  }
  ...
  public void processElement(ProcessContext c) {
    ...
    try {
      ...
      boolean skipShadowProcessing = false;
      if (!skipShadowProcessing) {
      ...
      if (!isSourceAhead) {
        ...
        InputRecordProcessor.processRecords(
            new ArrayList<
                com.google.cloud.teleport.v2.templates.common.TrimmedShardedDataChangeRecord>(
        ...
        if (!skipShadowProcessing) {
          try {
            ...
            LOG.error("Failed to update last commit timestamp 1", e);
            LOG.error("This will be written to DLQ");
          }
        }
      }
    } catch (Exception e) {
      LOG.error("Failed to update last commit timestamp 2", e);
      LOG.error("This will be written to DLQ");
    }
  }
  ...
  private com.google.cloud.spanner.Key changeEventToPrimaryKey(
    ...
    try {
      ...
      LOG.error("Failed to update last commit timestamp 3", e);
      LOG.error("This will be written to DLQ");
    }
    return pk.build();
  }
  ...
  private Mutation getShadowTableMutation(
    ...
    try {
      ...
      LOG.error("Failed to update last commit timestamp 4", e);
      LOG.error("This will be written to DLQ");
    }
