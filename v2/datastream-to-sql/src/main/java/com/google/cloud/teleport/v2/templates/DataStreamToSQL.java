...
/**
 * This pipeline ingests DataStream data from GCS. The data is then cleaned and converted from
 * JSON objects into DML statements. The DML is applied to the desired target database, which
 * can be one of MySQL or POstgreSQL.  Replication maintains a 1:1 match between source and target
 * by default.
...
public class DataStreamToSQL {
  ...
  public static CdcJdbcIO.DataSourceConfiguration getDataSourceConfiguration(Options options) {
    ...
    switch (options.getDatabaseType()) {
      case "postgres":
        ...
        jdbcDriverConnectionString = String.format(
  ...
  /**
   * Validate the options supplied match expected values.
   * We will also validate that connectivity is working correctly
   * for the target SQL database.
   *
   * @param options The execution parameters to the pipeline.
    ...
    dmlStatements
        .apply("Write to SQL",
            CdcJdbcIO.<DmlInfo>write()
                ...
                .withStatementFormatter(new CdcJdbcIO.StatementFormatter<DmlInfo>() {
