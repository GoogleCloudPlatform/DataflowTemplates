...
The [PubSubCdcToBigQuery] pipeline
ingests data from a PubSub subscription, optionally applies a JavaScript or Python UDF if supplied 
and writes the data to BigQuery.
...
export TOPIC=projects/${PROJECT}/topics/<topic-name>
export SUBSCRIPTION=projects/${PROJECT}/subscriptions/dataflow-subscription
export DATASET_TEMPLATE=webhook
export TABLE_NAME_TEMPLATE=flex_test
export DEADLETTER_TABLE=${PROJECT}:${DATASET_TEMPLATE}.dead_letter
...
echo '{
    ...
    "parameters":[
        {
            "name":"useSubscription","label":"Use PubSub Subscription",
            "helpText":"Use Subscription instead of creating a new one",
            "paramType":"TEXT"
        },
        {
            "name":"inputTopic",
            "label":"PubSub Topic Name",
            "helpText":"Name of PubSub topic in full reference format",
            "paramType":"TEXT"
        },
        {
            "name":"inputSubscription",
...
The template requires the following parameters:
* useSubscription: Boolean value representing if the pipeline should use a supplied Pub/Sub subscription
* inputTopic: The Pub/Sub Topic to read from (ie. projects/<project-id>/topics/<topic-name>)
* inputSubscription: PubSub subscription to read from (ie. projects/<project-id>/subscriptions/<subscription-name>)
* outputDatasetTemplate: The name of the dataset or templated logic to extract it (ie. 'prefix_{schema_name}')
* outputTableNameTemplate: The name of the table or templated logic to extract it (ie. 'prefix_{table_name}')
* outputDeadletterTable: Deadletter table for failed inserts in form: project-id:dataset.table
...
gcloud beta dataflow flex-template run ${JOB_NAME} \
        --project=${PROJECT} --region=us-central1 \
        --template-file-gcs-location=${TEMPLATE_IMAGE_SPEC} \
        --parameters useSubscription=true,inputTopic=${TOPIC},inputSubscription=${SUBSCRIPTION},outputDatasetTemplate=${DATASET_TEMPLATE},outputTableNameTemplate=${TABLE_NAME_TEMPLATE},outputDeadletterTable=${DEADLETTER_TABLE}
```
<<<<
