...
variable "inputSubscription" {
  type        = string
  description = "The Pub/Sub input subscription to read from. For example, `projects/<PROJECT_ID>/subscriptions/<SUBSCRIPTION_NAME>`"
...
variable "connectionUrl" {
  type        = string
  description = "The JDBC connection URL string. You can pass in this value as a string that's encrypted with a Cloud KMS key and then Base64-encoded. Remove whitespace characters from the Base64-encoded string. For example, `jdbc:mysql://some-host:3306/sampledb`"
...
variable "driverJars" {
  type        = string
  description = "Comma separated Cloud Storage paths for JDBC drivers.  For example, `gs://your-bucket/driver_jar1.jar,gs://your-bucket/driver_jar2.jar`"

}

variable "connectionProperties" {
  type        = string
  description = "The properties string to use for the JDBC connection. The string must use the format `[propertyName=property;]*`.  For example, `unicode=true;characterEncoding=UTF-8`"
  default     = null
}

variable "statement" {
  type        = string
  description = "The statement to run against the database. The statement must specify the column names of the table in any order. Only the values of the specified column names are read from the JSON and added to the statement.  For example, `INSERT INTO tableName (column1, column2) VALUES (?,?)`"
