...
variable "driverJars" {
  type        = string
  description = "The comma-separated list of driver JAR files. For example, `gs://your-bucket/driver_jar1.jar,gs://your-bucket/driver_jar2.jar`"
...
variable "connectionURL" {
  type        = string
  description = "The JDBC connection URL string. For example, `jdbc:mysql://some-host:3306/sampledb`. You can pass in this value as a string that's encrypted with a Cloud KMS key and then Base64-encoded. Remove whitespace characters from the Base64-encoded string. Note the difference between an Oracle non-RAC database connection string (`jdbc:oracle:thin:@some-host:<port>:<sid>`) and an Oracle RAC database connection string (`jdbc:oracle:thin:@//some-host[:<port>]/<service_name>`). For example, `jdbc:mysql://some-host:3306/sampledb`"
...
variable "query" {
  type        = string
  description = "The query to run on the source to extract the data. Note that some JDBC SQL and BigQuery types, although sharing the same name, have some differences. Some important SQL -> BigQuery type mappings to keep in mind are `DATETIME --> TIMESTAMP`. Type casting may be required if your schemas do not match. For example, `select * from sampledb.sample_table`"
  default     = null
}

variable "outputTable" {
  type        = string
  description = "The BigQuery output table location. For example, `<PROJECT_ID>:<DATASET_NAME>.<TABLE_NAME>`"

}

variable "bigQueryLoadingTemporaryDirectory" {
  type        = string
  description = "The temporary directory for the BigQuery loading process. For example, `gs://your-bucket/your-files/temp_dir`"

}

variable "KMSEncryptionKey" {
  type        = string
  description = "The Cloud KMS encryption key to use to decrypt the username, password, and connection string. If you  pass in a Cloud KMS key, you must also encrypt the username, password, and connection string. For example, `projects/your-project/locations/global/keyRings/your-keyring/cryptoKeys/your-key`"
  default     = null
}
...
variable "partitionColumn" {
  type        = string
  description = "If `partitionColumn` is specified along with the `table`, JdbcIO reads the table in parallel by executing multiple instances of the query on the same table (subquery) using ranges. Currently, supports `Long` and `DateTime` partition columns. Pass the column type through `partitionColumnType`."
  default     = null
}

variable "partitionColumnType" {
  type        = string
  description = "The type of the `partitionColumn`, accepts either `long` or `datetime`. Defaults to: long."
  default     = null
}

variable "table" {
  type        = string
  description = "The table to read from when using partitions. This parameter also accepts a subquery in parentheses. For example, `(select id, name from Person) as subq`"
  default     = null
}
...
variable "lowerBound" {
  type        = string
  description = "The lower bound to use in the partition scheme. If not provided, this value is automatically inferred by Apache Beam for the supported types. `datetime` partitionColumnType accepts lower bound in the format `yyyy-MM-dd HH:mm:ss.SSSZ`. For example, `2024-02-20 07:55:45.000+03:30`."
  default     = null
}

variable "upperBound" {
  type        = string
  description = "The upper bound to use in the partition scheme. If not provided, this value is automatically inferred by Apache Beam for the supported types. `datetime` partitionColumnType accepts upper bound in the format `yyyy-MM-dd HH:mm:ss.SSSZ`. For example, `2024-02-20 07:55:45.000+03:30`."
  default     = null
}
...
variable "bigQuerySchemaPath" {
  type        = string
  description = "The Cloud Storage path for the BigQuery JSON schema. If `createDisposition` is set to `CREATE_IF_NEEDED`, this parameter must be specified. For example, `gs://your-bucket/your-schema.json`"
  default     = null
}
...
resource "google_dataflow_flex_template_job" "generated" {
  ...
  parameters = {
    ...
    partitionColumnType               = var.partitionColumnType
    table                             = var.table
