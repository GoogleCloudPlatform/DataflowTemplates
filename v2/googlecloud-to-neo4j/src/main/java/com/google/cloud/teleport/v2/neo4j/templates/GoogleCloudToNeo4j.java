...
import com.google.cloud.teleport.v2.neo4j.actions.transforms.ActionBeamFactory;
...
public class GoogleCloudToNeo4j {
    ...
    public void run() {
        ...
        for (Source source : jobSpec.getSourceList()) {
            ...
            if (!targetsHaveTransforms || !providerImpl.supportsSqlPushDown()) {
                ...
                nullableSourceBeamRows = pipeline
                                .apply("Query "+source.getName(), providerImpl.querySourceBeamRows(sourceQuerySpec))
            ...
            for (Target nodeTarget : nodeTargets) {
                ...
                String nodeStepDescription=nodeTarget.getSequence() + ": "+source.getName()+"->"+nodeTarget.getName() +" nodes";
                if (ModelUtils.targetHasTransforms(nodeTarget)) {
                    preInsertBeamRows = pipeline.apply("Query "+nodeStepDescription,
                                    providerImpl.queryTargetBeamRows(targetQuerySpec));
                } else {
                    preInsertBeamRows = nullableSourceBeamRows;
                }

                Neo4jRowWriterTransform targetWriterTransform = new Neo4jRowWriterTransform(jobSpec, neo4jConnection, nodeTarget);

                PCollection<Row> blockingReturn =
                        preInsertBeamRows.apply(
                                        "** Unblocking " + nodeStepDescription + "(after "+ nodeTarget.getExecuteAfter() + "."+nodeTarget.getExecuteAfterName()+")",
                                        Wait.on(
            ...
            for (Target relationshipTarget : relationshipTargets) {
                TargetQuerySpec targetQuerySpec =
                        TargetQuerySpec.builder()
                                ...
                                .build();
                PCollection<Row> preInsertBeamRows;
                String relationshipStepDescription=relationshipTarget.getSequence() + ": "+source.getName()+"->"+relationshipTarget.getName() +" edges";
                if (ModelUtils.targetHasTransforms(relationshipTarget)) {
                    preInsertBeamRows =
                            pipeline.apply("Query " + relationshipStepDescription
                                    , providerImpl.queryTargetBeamRows(targetQuerySpec));
                } else {
                    preInsertBeamRows = nullableSourceBeamRows;
                }
                Neo4jRowWriterTransform targetWriterTransform = new Neo4jRowWriterTransform(jobSpec, neo4jConnection, relationshipTarget);

                PCollection<Row> blockingReturn =
                        preInsertBeamRows.apply(
                         "** Unblocking " + relationshipStepDescription + "(after "+ relationshipTarget.getExecuteAfter() + "."+relationshipTarget.getExecuteAfterName()+")",
                                Wait.on(
                                        processingQueue.waitOnCollection(
                ...
                processingQueue.addToQueue(
                        ArtifactType.edge, false, relationshipTarget.getName(), blockingReturn, preInsertBeamRows);
            }
        }
    ...
    private void runPreloadActions(List<Action> actions) {
        for (Action action : actions) {
            ...
            PreloadAction actionImpl = com.google.cloud.teleport.v2.neo4j.actions.ActionPreloadActionFactory.of(action, context);
    ...
    private void runBeamActions(List<Action> actions, BeamBlock blockingQueue) {
        for (Action action : actions) {
            ...
            context.jobSpec = this.jobSpec;
            context.neo4jConnectionParams = this.neo4jConnection;
            PTransform<PCollection<Row>, PCollection<Row>> actionImpl = ActionBeamFactory.of(context);
            ...
            PCollection<Row> actionTransformed =
                    executionContextCollection
                            .apply(
                                    "** Unblocking " + action.name + "(after "+ action.executeAfter + "."+action.executeAfterName+")",
                                    Wait.on(blockingQueue.waitOnCollection(action.executeAfter, action.executeAfterName, action.name)))
                            .setCoder(executionContextCollection.getCoder())
                            .apply("Running " + action.name, actionImpl);
