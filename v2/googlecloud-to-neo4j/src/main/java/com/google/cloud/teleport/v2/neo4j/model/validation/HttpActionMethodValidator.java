...
public class HttpActionMethodValidator implements SpecificationValidator {

  private static final String ERROR_CODE = "DFHT-001";
  ...
  public boolean report(Builder builder) {
    offendingActionIndices.forEach(
        index -> {
          String path = "$.actions[%d].method".formatted(index);
          builder.addError(path, ERROR_CODE, String.format("%s must be one of: GET, POST", path));
        });
