...
public class BigQueryActionSqlValidator implements SpecificationValidator {

  private static final String ERROR_CODE = "DFBQ-002";

  private final List<Integer> offendingActionIndices = new ArrayList<>();

  @Override
  public void visitAction(int index, Action action) {
    if (!(action instanceof BigQueryAction bigQueryAction)) {
      return;
    }
    if (bigQueryAction.sql().isBlank()) {
