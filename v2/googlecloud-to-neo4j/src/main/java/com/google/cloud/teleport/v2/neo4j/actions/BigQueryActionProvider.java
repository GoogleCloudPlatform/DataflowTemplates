...
public class BigQueryActionProvider implements ActionProvider<BigQueryAction> {
  ...
  @Override
  public BigQueryAction apply(ObjectNode objectNode) {
    return new BigQueryAction(
        getActive(objectNode),
        getName(objectNode),
        getActionStage(objectNode),
        objectNode.get("sql").asText(""));
  }
}
