...
gcloud dataflow flex-template run "test-bq-cli-`date +%Y%m%d-%H%M%S`" \
    ...
    --region "$REGION" \
    --parameters jobSpecUri="gs://neo4j-se-dataflow/job-specs/testing/bigquery/bq-northwind-jobspec.json" \
...
-d '{
   "launch_parameter": {
      ...
      "parameters": {
         "jobSpecUri": "gs://neo4j-se-dataflow/job-specs/testing/bq-northwind-jobspec.json",
