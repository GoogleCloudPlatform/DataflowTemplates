...
public class MergeRangesDoFnTest {
  ...
  @Test
  public void testMergeRangesDoFn() {
    ImmutableList<Range> testRanges = getDummyRanges(mockProcessContext);
    MergeRangesDoFn mergeRangesDoFn =
        MergeRangesDoFn.builder().setMaxPartitionHint(2L).setApproxTableCount(3200L).build();
    ...
    assertThat(rangesCaptor.getValue())
        .isEqualTo(
            ImmutableList.of(
                testRanges.get(0),
                // Can not merge after 0 int 1 as count exceeds mean.
                testRanges
                    .get(1)
                    .mergeRange(testRanges.get(2), mockProcessContext)
                    .mergeRange(testRanges.get(3), mockProcessContext),
                // Can not merge after 3 int 4 as the ranges are not mergable.
