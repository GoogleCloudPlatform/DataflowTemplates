...
public class JsonNodeToMutationDoFnTest {
  ...
  public void testNodeToMutation_basic() throws Exception {
    ...
    JsonNode changeEvent =
        mapper.readTree(
            String.format(
                "{\"%s\":\"cart\", \"product_id\":\"product1\", \"quantity\": 5, \"user_id\": \"user1\"}",
                Constants.EVENT_TABLE_NAME_KEY));
    List<JsonNode> inputList = ImmutableList.of(changeEvent);
    PCollection<JsonNode> input =
        pipeline.apply("input", Create.of(inputList).withCoder(JsonNodeCoder.of()));
    PCollection<Mutation> mutations =
        input.apply(
            "Map JsonNode to Mutations",
            ParDo.of(new JsonNodeToMutationDoFn(schema, "cart", getTestDdl())));
  ...
  public void testNodeToMutation_exception() throws Exception {
    ...
    JsonNode changeEvent =
        mapper.readTree(
            String.format(
                "{\"%s\":\"cart\",\"quantity\": \"hello\"}", Constants.EVENT_TABLE_NAME_KEY));
    List<JsonNode> inputList = ImmutableList.of(changeEvent);
    PCollection<JsonNode> input =
        pipeline.apply("input", Create.of(inputList).withCoder(JsonNodeCoder.of()));
    PCollection<Mutation> mutations =
