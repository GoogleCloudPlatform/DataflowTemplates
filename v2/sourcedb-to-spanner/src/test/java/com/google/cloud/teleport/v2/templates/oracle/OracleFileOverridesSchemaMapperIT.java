...
public class OracleFileOverridesSchemaMapperIT extends SourceDbToSpannerITBase {
  ...
  private static final String ORACLE_DDL_RESOURCE =
      "oracle/OracleFileOverridesSchemaMapperIT/oracle-schema.sql";
  private static final String SPANNER_DDL_RESOURCE =
      "oracle/OracleFileOverridesSchemaMapperIT/oracle-GOOGLE_STANDARD_SQL-spanner-schema.sql";
  private static final String SCHEMA_OVERRIDE_FILE_RESOURCE = "SchemaMapperIT/file-overrides.json";
  private static final String SCHEMA_OVERRIDE_GCS_PREFIX = "OracleFileOverridesCommonIT";
  ...
  public void setUp() throws Exception {
    ...
    String oracleJarPath =
        Arrays.stream(System.getProperty("java.class.path").split(":"))
            .filter(p -> p.contains("ojdbc8") && p.endsWith(".jar"))
            .findFirst()
            .orElseThrow(() -> new RuntimeException("ojdbc8 jar not found in classpath"));
    gcsResourceManager.uploadArtifact(SCHEMA_OVERRIDE_GCS_PREFIX + "/ojdbc8.jar", oracleJarPath);
  }
  ...
  public void testMigrationWithFileOverridesAndCommonSchema() throws Exception {
    ...
    jobParameters.put(
        "jdbcDriverJars",
        getGcsPath(SCHEMA_OVERRIDE_GCS_PREFIX + "/ojdbc8.jar", gcsResourceManager));
