...
public class SessionSchemaMapperIT extends SourceDbToSpannerITBase {
  ...
  private static final String SESSION_FILE_RESOURCE =
      "SingleShardWithTransformation/company-session.json";
  ...
  private static final String SESSION_FILE_WITH_TRANSFORMATION_RESOURCE =
      "SingleShardWithTransformation/company-session-with-transformation.json";
  ...
  public void transformationTest() throws Exception {
    ...
    ImmutableList<Struct> companySpanner =
        spannerResourceManager.readTableRecords(
            "company", "company_id", "company_name_sp", "created_on");
    ...
    List<Map<String, Object>> employeeMySQL =
        mySQLResourceManager.runSQLQuery(
            "SELECT employee_id, company_id, employee_name, employee_address as employee_address_sp, created_on FROM employee");
    ImmutableList<Struct> employeeSpanner =
        spannerResourceManager.readTableRecords(
            "employee",
            "employee_id",
    ...
    ImmutableList<Struct> employeeAttribute =
        spannerResourceManager.readTableRecords(
            "employee_attribute", "employee_id", "attribute_name", "value", "updated_on");
    SpannerAsserts.assertThatStructs(employeeAttribute)
        .hasRows(0); // As string keys are not supported currently
  }

  @Test
  public void transformationTestWithTableFilter() throws Exception {
    loadSQLFileResource(mySQLResourceManager, MYSQL_DDL_RESOURCE);
    createSpannerDDL(spannerResourceManager, SPANNER_DDL_WITH_TRANSFORMATION_RESOURCE);
