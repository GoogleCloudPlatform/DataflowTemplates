...
public class BoundarySplitterFactory {
  ...
  private static Integer splitIntegers(Integer start, Integer end) {
    ...
    // This is same as (start + (end - start)/2) but that the width calculation can overflow if
    // start is INTEGER.MIN and end is INTEGER.MAX.
  ...
  private static Long splitLongs(Long start, Long end) {
    ...
    // This is same as (start + (end - start)/2) but that the width calculation can overflow if
    // start is LONG.MIN and end is LONG.MAX.
    return (start & end) + ((start ^ end) >> 1);
  }
