...
public class JsonNodeToMutationDoFn extends DoFn<JsonNode, Mutation> implements Serializable {
  ...
  public JsonNodeToMutationDoFn(Schema schema, String srcTable, Ddl ddl) {
    this.schema = schema;
  ...
  public void processElement(ProcessContext c) {
    // We create a copy because Beam does not allow mutations on the input element (i.e c.element())
    // for unit tests.
    ...
    try {
      ...
      Mutation mutation =
          ChangeEventSpannerConvertor.mutationFromEvent(
              table, changeEvent, columnNames, keyColumns);
      c.output(mutation);
    } catch (Exception ex) {
