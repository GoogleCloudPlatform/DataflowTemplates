 ...
 * the License.
 */
package com.google.cloud.teleport.v2.source.jdbc.iowrapper;

import static com.google.cloud.teleport.v2.source.reader.io.schema.SchemaDiscoveryImpl.convertException;
...
import com.google.cloud.teleport.v2.source.jdbc.dialectadapter.DialectAdapter;
...
import com.google.cloud.teleport.v2.source.jdbc.iowrapper.config.TableConfig;
import com.google.cloud.teleport.v2.source.jdbc.rowmapper.JdbcSourceRowMapper;
import com.google.cloud.teleport.v2.source.jdbc.uniformsplitter.DataSourceProvider;
...
import com.google.cloud.teleport.v2.source.jdbc.uniformsplitter.transforms.ReadWithUniformPartitions;
...
public final class JdbcIoWrapper implements IoWrapper {
   ...
   *     column.
   */
  public static JdbcIoWrapper of(JdbcIoWrapperConfigGroup configGroup, ISourceConnector connector)
      throws SuitableIndexNotFoundException {

    ImmutableList<PerSourceDiscovery> perSourceDiscoveries =
        getPerSourceDiscoveries(configGroup, connector);
  ...
  protected static ImmutableList<PerSourceDiscovery> getPerSourceDiscoveries(
      JdbcIoWrapperConfigGroup configGroup, ISourceConnector connector) {
    ...
    try {
      List<Future<PerSourceDiscovery>> futures =
          configGroup.shardConfigs().stream()
              .map(config -> executor.submit(() -> getPerSourceDiscovery(config, connector)))
   ...
   */
  private static PerSourceDiscovery getPerSourceDiscovery(
      JdbcIOWrapperConfig config, ISourceConnector connector) {
    ...
    try {
      setDataSourceLoginTimeout((BasicDataSource) dataSource, config, connector);
  ...
  protected static void setDataSourceLoginTimeout(
      BasicDataSource dataSource, JdbcIOWrapperConfig config, ISourceConnector connector) {
    connector.setDataSourceLoginTimeout(
