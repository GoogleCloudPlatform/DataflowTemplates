 ...
 * com.google.cloud.teleport.v2.source.jdbc.uniformsplitter.range.Range} objects), identifies their
 * source table, and executes the appropriate SQL query dynamically.
 *
 ...
 * those within {@link ReadWithUniformPartitions}) for collation detection, consume side-inputs
 * generated in the {@link org.apache.beam.sdk.transforms.windowing.GlobalWindow}. In Apache Beam,
 * side-inputs in the Global Window are only considered "ready" at the end of the window ({@code
 * TIMESTAMP_MAX_VALUE}). Consequently, any element produced by a transform consuming such a
 * side-input inherits this "End of Time" timestamp (approx. year 294247). This can cause downstream
 * failures, specifically in the Dead Letter Queue (DLQ) path. To address this, {@link TimeStampRow}
 * is used to explicitly reset the timestamp of elements to the current system time.
 *
