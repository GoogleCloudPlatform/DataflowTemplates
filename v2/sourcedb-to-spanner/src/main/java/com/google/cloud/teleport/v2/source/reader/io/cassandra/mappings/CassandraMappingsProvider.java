...
import com.google.cloud.teleport.v2.source.reader.io.schema.typemapping.provider.unified.CustomSchema.Epoch;
...
public class CassandraMappingsProvider {
   ...
   * for further details.
   */
  private static final RowValueMapper<Long> timeToEpoch =
      (value, schema) ->
          new GenericRecordBuilder(Epoch.SCHEMA)
              .set(Epoch.SECONDS_FIELD_NAME, 0L)
              .set(Epoch.NANOS_FIELD_NAME, value)
              .build();
  ...
  private static final CassandraMappings CASSANDRA_MAPPINGS =
      CassandraMappings.builder()
          ...
          .put("DURATION", UnifiedMappingProvider.Type.DURATION_NANO, getDuration, durationToAvro)
          ...
          .put("TIME", UnifiedMappingProvider.Type.EPOCH, Row::getTime, timeToEpoch)
