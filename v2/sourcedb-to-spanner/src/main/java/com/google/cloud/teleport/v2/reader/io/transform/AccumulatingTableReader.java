 ...
 * the License.
 */
package com.google.cloud.teleport.v2.reader.io.transform;
...
abstract class AccumulatingTableReader extends PTransform<PBegin, PCollectionTuple> {
    ...
    PCollection<SourceTableReference> tableCompletions =
        PCollectionList.of(
                tablePCollections.entrySet().stream()
                    .map(
                        e -> {
                          return getSourceTableReference(input, e);
                        })
  ...
  @NotNull
  private static PCollection<SourceTableReference> getSourceTableReference(
      PBegin input, Entry<ImmutableList<SourceTableReference>, PCollection<SourceRow>> e) {
    ...
    return PCollectionList.of(actualCounts)
        ...
        .apply(
            "GenerateCompletions." + groupName, ParDo.of(new GroupCompletionDoFn(tableReferences)));
  }
