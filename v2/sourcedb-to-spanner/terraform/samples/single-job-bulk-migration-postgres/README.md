...
1. Dataflow can establish network connectivity with the PostgreSQL source.
...
Given these assumptions, it copies data from the source PostgreSQL database to the configured Spanner database.
...
- **Job logs not present after "Discovering tables for DataSource":** This means dataflow is unable to access the postgresql
...
Ensure Dataflow VMs are able to access the PostgreSQL instance. This would require:

- Configuring PostgreSQL to allow connections from the Dataflow IPs.
- Configuring the firewall to allow ingress TCP connection on the PostgreSQL port
  from the Dataflow IPs.

#### Configuring connectivity on PostgreSQL on GCE
...
- Using network tags to configure PostgreSQL VM. Apply the `databases` network
  tag on the GCE VM.
- Add firewall rule that allows tcp ingress on port 5432 from resources
...
For example, for PostgreSQL running on GCE, one can specify the VPC network and subnetwork
of the GCE instance hosting PostgreSQL.
