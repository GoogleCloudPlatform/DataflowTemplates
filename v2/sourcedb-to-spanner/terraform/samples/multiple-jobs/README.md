## Sample Scenario: MySQL to Spanner production bulk migrations

> **_SCENARIO:_** This Terraform example illustrates launching a bulk (backfill/historical) migration Dataflow job(s)
> from
> MySQL to Spanner.
...
roles. Also ensure that the workers can access the sources.
...
1. Dataflow can establish network connectivity with the MySQL sources.
...
5. A Spanner instance with database containing the data-migration compatible schema is created. If it is not
   like-to-like i.e. there are modifications in the Spanner schema, provide an SMT generated session file is mandatory.
   Check out the FAQ on [how to generate a session file](#specifying-schema-overrides).
...
1. **Dataflow job(s)** - The Dataflow job(s) which reads from source databases and writes to
   Spanner.
2. **GCS Session File** - If provided, the GCS object created by uploading the local session file.
...
- If you are still getting a Timeout after this, this means the job graph construction is taking too long. You could
  potentially have too many tables allocated in a single Dataflow job The recommended limit is 150.
...
- **Check source database metrics:** Are memory/CPU limits being hit? Consider increasing the source db resources.
...
source and Spanner. However, for a sharded migration or if schema was modified when moving to Spanner, a session file is
