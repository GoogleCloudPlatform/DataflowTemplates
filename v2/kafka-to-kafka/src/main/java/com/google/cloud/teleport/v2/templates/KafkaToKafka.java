    ...
    if (options.getMigrationType() != null) {
      ...
      if (options.getSourceAuthenticationMethod() != null) {
        ...
        records =
            pipeline.apply(
                ...
                KafkaIO.<byte[], byte[]>read()
                    ...
                    .withConsumerConfigUpdates(options.getSourceAuthenticationMethod()==null ? ImmutableMap.of() : ConsumerProperties.get(options))
                    .withoutMetadata());
        ...
        records.apply(
            ...
            kafkaWrite.withProducerConfigUpdates(options.getDestinationAuthenticationMethod()==null ? ProducerProperties.get(options) : ImmutableMap.of(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest")));
      } else {
        PCollection<KV<byte[], byte[]>> records;
        records =
            pipeline.apply(
