...
/**
 * Factory classes for ChangeEventSequence classes which provides methods for
 * 1) creating ChangeEventSequence objects for the current ChangeEvent.
 * 2) creating ChangeEventSequence objects for an earlier ChangeEvent
 * by reading from shadow table.
 */
  ...
  public static ChangeEventSequence createChangeEventSequenceFromShadowTable(
    ...
    if (DatastreamConstants.MYSQL_SOURCE_TYPE.equals(sourceType)) {
      return MySqlChangeEventSequence.createFromShadowTable(
          transactionContext, changeEventContext.getShadowTable(),
