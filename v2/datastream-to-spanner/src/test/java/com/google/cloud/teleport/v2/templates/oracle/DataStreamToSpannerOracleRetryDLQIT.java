...
public class DataStreamToSpannerOracleRetryDLQIT extends DataStreamToSpannerITBase {
  ...
  private CloudOracleResourceManager setUpOracleResourceManagerForShard(String shardName) {
    if (System.getProperty("hostIp") != null) {
      CloudOracleResourceManager.Builder shardBuilder = CloudOracleResourceManager.builder(testName + shardName);
      ...
      if (jobInfo == null) {
        ...
        OracleSource oracleSourceA =
            OracleSource.builder(
                    ...
                    jdbcResourceManagerShardA.getPort(), "XEPDB1")
                .setAllowedTables(
                    Map.of(
                        jdbcResourceManagerShardA.getUsername().toUpperCase(),
                        List.of("Customers", "Orders", "AllDataTypes"))) // Assuming schemas are username
                .build();
  ...
  public void testDataStreamToSpannerShardedRetryDLQ() throws Exception {
    ...
    assertTrue("id=1 should exist in AllDataTypes on shard1",
        rowExistsInSpanner("AllDataTypes", "id", 1, "shard1"));
    assertTrue("id=999 should NOT exist yet in AllDataTypes on shard1",
        !rowExistsInSpanner("AllDataTypes", "id", 999, "shard1"));

    assertTrue("id=2 should exist in Customers on shard1",
        rowExistsInSpanner("Customers", "CustomerId", 2, "shard1"));
    assertTrue("id=1 should NOT exist yet in Customers on shard1",
        !rowExistsInSpanner("Customers", "CustomerId", 1, "shard1"));

    assertTrue("id=102 should exist in Orders on shard1",
        rowExistsInSpanner("Orders", "OrderId", 102, "shard1"));
    assertTrue("id=101 should NOT exist yet in Orders on shard1",
        !rowExistsInSpanner("Orders", "OrderId", 101, "shard1"));
    assertTrue("id=103 should NOT exist yet in Orders on shard1",
    ...
    assertTrue("id=1 should NOT exist", !rowExistsInSpanner("Customers", "CustomerId", 1, "shard1"));
