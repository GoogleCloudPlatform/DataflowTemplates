...
public class OracleSeparateShadowTableDatabasePKFocusedIT extends DataStreamToSpannerITBase {
  private static final Logger LOG = LoggerFactory.getLogger(OracleSeparateShadowTableDatabasePKFocusedIT.class);

  private static final String ORACLE_SCHEMA = "oracle/OracleSeparateShadowTableDatabasePKFocusedIT/oracle-schema.sql";
  private static final String SPANNER_DDL_RESOURCE = "oracle/OracleSeparateShadowTableDatabasePKFocusedIT/oracle-google_standard_sql-spanner-schema.sql";
  ...
  private static HashSet<OracleSeparateShadowTableDatabasePKFocusedIT> testInstances = new HashSet<>();
  private static PipelineLauncher.LaunchInfo jobInfo;

  private void setUpOracleUser(String user, String password) {
    oracleSysUser.runSQLUpdate(String.format("CREATE USER %s IDENTIFIED BY %s CONTAINER=ALL", user, password));
    oracleSysUser.runSQLUpdate(String.format("GRANT EXECUTE_CATALOG_ROLE TO %s CONTAINER=ALL", user));
    ...
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$DATABASE TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$PDBS TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON DBA_SUPPLEMENTAL_LOGGING TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$ARCHIVED_LOG TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$LOGMNR_CONTENTS TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$LOG TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$LOGFILE TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$THREAD TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$PARAMETER TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$NLS_PARAMETERS TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$TIMEZONE_NAMES TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$LOGMNR_LOGS TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$ARCHIVE_DEST_STATUS TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.V_$TRANSACTION TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON SYS.DBA_REGISTRY TO %s CONTAINER=ALL", user));
    ...
    oracleSysUser.runSQLUpdate(String.format("GRANT UNLIMITED TABLESPACE TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ANY DICTIONARY TO %s CONTAINER=ALL", user));
    ...
    oracleSysUser.runSQLUpdate(String.format("GRANT EXECUTE ON DBMS_LOGMNR TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT EXECUTE ON DBMS_LOGMNR_D TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ANY TRANSACTION TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ANY TABLE TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT SELECT ON DBA_EXTENTS TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("GRANT CREATE ANY TABLE TO %s CONTAINER=ALL", user));
    oracleSysUser.runSQLUpdate(String.format("ALTER USER %s QUOTA 50m ON USERS CONTAINER=ALL", user));
  ...
  public void setUp() throws IOException {
    ...
    synchronized (OracleSeparateShadowTableDatabasePKFocusedIT.class) {
      ...
      if (!initialized) {
        ...
        oracleSysUser = (org.apache.beam.it.gcp.cloudsql.CloudOracleResourceManager) sysBuilder.build();
        ...
        oracleResourceManager = (org.apache.beam.it.gcp.cloudsql.CloudOracleResourceManager) builder.build();
        ...
        OracleSource oracleSource =
            OracleSource.builder(
                ...
                .setAllowedTables(
                    Map.of(oracleResourceManager.getUsername().toUpperCase(), List.of(ORACLE_TABLE_MY_TABLE, ORACLE_TABLE_ALLTYPES)))
                .build();
  ...
  private void flushOracleLogs() {
    try (java.sql.Connection conn = java.sql.DriverManager.getConnection(
            "jdbc:oracle:thin:@" + System.getProperty("hostIp") + ":1521/XE", "system", System.getProperty("cloudProxyPassword"));
  ...
  public void migrationTestSimpleTable() throws Exception {
    ConditionCheck insertRecords = new ConditionCheck() {
      boolean executed = false;
      @Override
      public String getDescription() { return "Insert 3 records into MY_TABLE"; }
      @Override
      protected CheckResult check() {
        if (!executed) {
            try {
              ...
              flushOracleLogs();
              executed = true;
            } catch (Exception e) { return new CheckResult(false, e.getMessage()); }
        }
        return new CheckResult(true, "Data inserted");
      }
    };
    
    ConditionCheck updateDeleteRecords = new ConditionCheck() {
      boolean executed = false;
      @Override
      public String getDescription() { return "Update and Delete in MY_TABLE"; }
      @Override
      protected CheckResult check() {
        if (!executed) {
            try {
    ...
    ConditionCheck pkUpdateRecords = new ConditionCheck() {
      boolean executed = false;
      @Override
      public String getDescription() { return "PK Update in MY_TABLE"; }
      @Override
      protected CheckResult check() {
        if (!executed) {
            try {
    ...
    ChainedConditionCheck conditionCheck =
        ChainedConditionCheck.builder(
                List.of(
                    ...
                    new ConditionCheck() {
                      @Override
                      public String getDescription() { return "Check if MY_TABLE PK updated to 10"; }
                      @Override
                      protected CheckResult check() {
                        try {
                          com.google.common.collect.ImmutableList<com.google.cloud.spanner.Struct> rows = spannerResourceManager.runQuery("SELECT ID FROM MY_TABLE WHERE ID = 10");
                          if (!rows.isEmpty()) {
    ...
    try { Thread.sleep(CUTOVER_MILLIS); } catch (Exception e) {}
    ...
    SpannerAsserts.assertThatStructs(spannerResourceManager.runQuery("select ID, VAL from MY_TABLE"))
  ...
  public void migrationTestAllDataType() throws Exception {
    ConditionCheck insertRecords = new ConditionCheck() {
      boolean executed = false;
      @Override
      public String getDescription() { return "Insert records into ALLTYPES"; }
      @Override
      protected CheckResult check() {
        if (!executed) {
            try {
              // 1
              oracleResourceManager.runSQLUpdate("INSERT INTO ALLTYPES (BOOL_FIELD, INT64_FIELD, FLOAT64_FIELD, STRING_FIELD, BYTES_FIELD, TIMESTAMP_FIELD, DATE_FIELD, NUMERIC_FIELD, VAL) " +
    ...
    ConditionCheck updateDeleteRecords = new ConditionCheck() {
      boolean executed = false;
      @Override
      public String getDescription() { return "Update and Delete in ALLTYPES"; }
      @Override
      protected CheckResult check() {
        if (!executed) {
            try {
              oracleResourceManager.runSQLUpdate("DELETE FROM ALLTYPES WHERE INT64_FIELD = 1");
              oracleResourceManager.runSQLUpdate("UPDATE ALLTYPES SET VAL = 10 WHERE INT64_FIELD = 2");
              flushOracleLogs();
              executed = true;
            } catch (Exception e) { return new CheckResult(false, e.getMessage()); }
        }
    ...
    ConditionCheck pkUpdateRecords = new ConditionCheck() {
      boolean executed = false;
      @Override
      public String getDescription() { return "Update PK in ALLTYPES"; }
      @Override
      protected CheckResult check() {
        if (!executed) {
            try {
              oracleResourceManager.runSQLUpdate("UPDATE ALLTYPES SET INT64_FIELD = 10 WHERE INT64_FIELD = 3");
              flushOracleLogs();
              executed = true;
            } catch (Exception e) { return new CheckResult(false, e.getMessage()); }
        }
    ...
    ChainedConditionCheck conditionCheck =
        ChainedConditionCheck.builder(
                List.of(
                    ...
                    new ConditionCheck() {
                      @Override
                      public String getDescription() { return "Check if ALLTYPES PK updated to 10"; }
                      @Override
                      protected CheckResult check() {
                        try {
                          com.google.common.collect.ImmutableList<com.google.cloud.spanner.Struct> rows = spannerResourceManager.runQuery("SELECT INT64_FIELD FROM ALLTYPES WHERE INT64_FIELD = 10");
                          if (!rows.isEmpty()) {
                            return new CheckResult(true, "PK updated to 10");
                          }
                        } catch (Exception e) {}
    ...
    SpannerAsserts.assertThatStructs(spannerResourceManager.runQuery("select INT64_FIELD, VAL from ALLTYPES"))
        .hasRecordsUnorderedCaseInsensitiveColumns(events);
  }
}
