...
public class OracleDataStreamToSpannerMixedIT extends DataStreamToSpannerITBase {
  ...
  public void mixedMigrationTest() throws IOException {
    ...
    createSpannerDDL(spannerResourceManager, "oracle/OracleDataStreamToSpannerMixedIT/oracle-google_standard_sql-spanner-schema.sql");

    org.apache.beam.it.gcp.cloudsql.CloudOracleResourceManager.Builder builder = org.apache.beam.it.gcp.cloudsql.CloudOracleResourceManager.builder(testName);
    ...
    executeSqlScript(cloudSqlResourceManager, "oracle/OracleDataStreamToSpannerMixedIT/oracle-schema.sql");
    ...
    DestinationConfig destinationConfig =
        datastreamResourceManager.buildGCSDestinationConfig(
            "gcs-profile", gcsResourceManager.getBucket(), gcsPrefix, DatastreamResourceManager.DestinationOutputFormat.AVRO_FILE_FORMAT);

    Stream stream =
        datastreamResourceManager.createStream("stream" + RandomStringUtils.randomAlphanumeric(5).toLowerCase(), sourceConfig, destinationConfig);
    ...
    PipelineLauncher.LaunchInfo jobInfo = launchDataflowJob(
        testName,
    ...
    ChainedConditionCheck conditionCheck =
        ChainedConditionCheck.builder(
                List.of(
                    writeInitialData(),
                    SpannerRowsCheck.builder(spannerResourceManager, "Authors").setMinRows(4).build(),
                    SpannerRowsCheck.builder(spannerResourceManager, "Books").setMinRows(3).build()))
            .build();

    PipelineOperator.Result result =
        pipelineOperator()
            .waitForConditionAndCancel(
                createConfig(jobInfo, Duration.ofMinutes(JOB_START_PROCESSING_WAIT_MINUTES + 10)), conditionCheck);
