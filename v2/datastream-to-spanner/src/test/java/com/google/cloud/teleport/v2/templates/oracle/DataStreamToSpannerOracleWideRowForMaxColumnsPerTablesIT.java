...
public class DataStreamToSpannerOracleWideRowForMaxColumnsPerTablesIT extends DataStreamToSpannerITBase {
  ...
  static {
    for (int i = 1; i <= NUM_TABLES; i++) {
        // According to instructions on quoting/identifiers: Oracle often folds to uppercase but Datastream captures whatever it is. Oh wait, Oracle tables are usually uppercase or need quotes.
        // Let's use uppercase to avoid case folding issues if unquoted
      TABLE_NAMES.add("DATASTREAMTOSPANNER_" + i + "_" + RandomStringUtils.randomAlphanumeric(5).toUpperCase());
    }
  ...
  public void setUp() throws IOException {
    ...
    synchronized (DataStreamToSpannerOracleWideRowForMaxColumnsPerTablesIT.class) {
      ...
      if (jobInfo == null) {
        ...
        cloudOracleResourceManager = (org.apache.beam.it.gcp.cloudsql.CloudOracleResourceManager) builder.build();

        String sessionContent = generateBaseSchema();
        sessionContent = sessionContent.replaceAll("SRC_DATABASE", cloudOracleResourceManager.getUsername().toUpperCase())
