...
public class OracleDataStreamToSpannerWideRowForMaxTableNameWithMaxColumnNameIT
  ...
  static {
    ...
    COLUMNS.add(
        NUM_COLUMNS - 1, "col_" + (NUM_COLUMNS - 1) + RandomStringUtils.randomAlphanumeric(25));
  }
  ...
  public void setUp() throws Exception {
    ...
    synchronized (OracleDataStreamToSpannerWideRowForMaxTableNameWithMaxColumnNameIT.class) {
      ...
      if (jobInfo == null) {
        ...
        sessionContent =
            sessionContent.replaceAll("SP_DATABASE", spannerResourceManager.getDatabaseId());
        for (int i = 1; i <= NUM_TABLES; i++) {
  ...
  public void cleanUp() throws IOException {
    for (OracleDataStreamToSpannerWideRowForMaxTableNameWithMaxColumnNameIT instance :
        testInstances) {
  ...
  private String getJDBCSchema(String tableName) {
    ...
    sb.append("CREATE TABLE \"")
        ...
        .append("\" (");
    for (int i = 1; i <= NUM_COLUMNS; i++) {
      sb.append("\"").append(COLUMNS.get(i - 1)).append("\" NUMERIC NOT NULL");
      if (i != NUM_COLUMNS) {
        sb.append(", ");
      }
    }
  ...
  private void createSpannerTables() {
    for (String tableName : TABLE_NAMES) {
      ...
      columns.add(COLUMNS.get(0) + " NUMERIC NOT NULL");

      for (int i = 2; i <= NUM_COLUMNS; i++) {
        columns.add(COLUMNS.get(i - 1) + " NUMERIC");
      }
        ...
        for (String tableName : TABLE_NAMES) {
          ...
          for (int i = 0; i < NUM_EVENTS; i++) {
            ...
            for (int ci = 1; ci <= NUM_COLUMNS; ci++) {
              ...
              cols.append("\"").append(COLUMNS.get(ci - 1)).append("\"");
              vals.append(ci);
              if (ci != NUM_COLUMNS) {
                cols.append(", ");
                vals.append(", ");
