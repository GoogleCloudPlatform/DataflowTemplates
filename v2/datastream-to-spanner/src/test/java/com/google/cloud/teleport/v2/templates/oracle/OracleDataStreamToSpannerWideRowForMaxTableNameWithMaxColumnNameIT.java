...
public class OracleDataStreamToSpannerWideRowForMaxTableNameWithMaxColumnNameIT
  ...
  public void setUp() throws IOException {
    ...
    synchronized (OracleDataStreamToSpannerWideRowForMaxTableNameWithMaxColumnNameIT.class) {
      ...
      if (jobInfo == null) {
        ...
        oracleSchema = ("TEST_U" + org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(5)).toUpperCase();
        cloudSqlResourceManager.runSQLUpdate(String.format("CREATE USER %s IDENTIFIED BY %s1A", oracleSchema, oracleSchema));
        ...
        sessionContent = sessionContent.replaceAll("SP_DATABASE", spannerResourceManager.getDatabaseId());
        for (int i = 1; i <= NUM_TABLES; i++) {
        ...
        jobInfo =
            launchDataflowJob(
                ...
                org.apache.beam.it.gcp.datastream.OracleSource.builder(
                        ...
                        cloudSqlResourceManager.getPort(), cloudSqlResourceManager.getDatabaseName())
                    .setAllowedTables(
                        Map.of(oracleSchema, TABLE_NAMES))
                    .build());
      }
    }
  }
  ...
  public void cleanUp() throws IOException {
    for (OracleDataStreamToSpannerWideRowForMaxTableNameWithMaxColumnNameIT instance : testInstances) {
  ...
  private void setupSchema() {
    TABLE_NAMES.forEach(
        tableName -> {
          ...
          cloudSqlResourceManager.runSQLUpdate("ALTER TABLE \"" + oracleSchema + "\".\"" + tableName + "\" ADD SUPPLEMENTAL LOG DATA (ALL) COLUMNS");
        });
    createSpannerTables();
  }
  ...
  private String getJDBCSchema(String tableName) {
    ...
    sb.append("CREATE TABLE \"").append(oracleSchema).append("\".\"").append(tableName).append("\" (");
    for (int i = 1; i <= NUM_COLUMNS; i++) {
      ...
      protected CheckResult check() {
        ...
        for (String tableName : TABLE_NAMES) {
          ...
          for (int i = 0; i < NUM_EVENTS; i++) {
            ...
            rows.add(values);
            String sql = "INSERT INTO \"" + oracleSchema + "\".\"" + tableName + "\" (" + cols.toString() + ") VALUES (" + vals.toString() + ")";
            try { cloudSqlResourceManager.runSQLUpdate(sql); cloudSqlResourceManager.runSQLUpdate("COMMIT"); } catch (Exception e) { success = false; }
          }
