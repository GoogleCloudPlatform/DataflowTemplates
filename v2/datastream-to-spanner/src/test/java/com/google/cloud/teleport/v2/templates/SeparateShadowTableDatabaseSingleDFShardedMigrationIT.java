...
/**
 * Integration test using separate shadow table database for sharded data migration using a single
 * Dataflow job per migration. In this scenario, one Datastream job is created for each physical
 * shard, all of which share the same target connection profile, writing to the same GCS bucket. As
 * a result, only one Pub/Sub notification is generated per migration, triggering a single Dataflow
 * job. This test also validates the addition of the migration_shard_id column to the schema for
 * each table in the {@link DataStreamToSpanner} Flex template, while using a single Dataflow job
 * for the entire migration.
 */
@Category({TemplateIntegrationTest.class, SkipDirectRunnerTest.class})
@TemplateIntegrationTest(DataStreamToSpanner.class)
@RunWith(JUnit4.class)
public class SeparateShadowTableDatabaseSingleDFShardedMigrationIT
    extends DataStreamToSpannerITBase {
  private static final Logger LOG =
      LoggerFactory.getLogger(SeparateShadowTableDatabaseSingleDFShardedMigrationIT.class);
  ...
  private static HashSet<SeparateShadowTableDatabaseSingleDFShardedMigrationIT> testInstances =
      new HashSet<>();
  ...
  public void setUp() throws IOException, InterruptedException {
    ...
    synchronized (SeparateShadowTableDatabaseSingleDFShardedMigrationIT.class) {
      testInstances.add(this);
      if (spannerResourceManager == null) {
        spannerResourceManager = setUpSpannerResourceManager();
        createSpannerDDL(spannerResourceManager, SPANNER_DDL_RESOURCE);
      }
      if (shadowSpannerResourceManager == null) {
