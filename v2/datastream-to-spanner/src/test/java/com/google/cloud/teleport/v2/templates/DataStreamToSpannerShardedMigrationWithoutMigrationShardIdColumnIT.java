...
/** Sharded data migration Integration test without any migration_shard_id column transformation
 * for {@link DataStreamToSpanner} Flex template. */
@Category({TemplateIntegrationTest.class, SkipDirectRunnerTest.class})
@TemplateIntegrationTest(DataStreamToSpanner.class)
@RunWith(JUnit4.class)
public class DataStreamToSpannerShardedMigrationWithoutMigrationShardIdColumnIT extends DataStreamToSpannerITBase {
  private static final Logger LOG =
      LoggerFactory.getLogger(DataStreamToSpannerShardedMigrationWithoutMigrationShardIdColumnIT.class);

  private static final String TABLE = "Users";
  private static final String SPANNER_DDL_RESOURCE =
      "DataStreamToSpannerShardedMigrationWithoutMigrationShardIdColumnIT/spanner-schema.sql";

  private static HashSet<DataStreamToSpannerShardedMigrationWithoutMigrationShardIdColumnIT> testInstances = new HashSet<>();
  ...
  public static void cleanUp() throws IOException {
    for (DataStreamToSpannerShardedMigrationWithoutMigrationShardIdColumnIT instance : testInstances) {
