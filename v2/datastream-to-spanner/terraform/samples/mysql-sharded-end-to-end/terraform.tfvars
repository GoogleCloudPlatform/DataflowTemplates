...
common_params = {
  ...
  # Replace with your desired GCP region (e.g., "us-central1")
  ...
  datastream_params = {
    ...
    private_connectivity = {
      ...
      # The VPC network name to attach Private Service Connect to
      range                   = "<YOUR_IP_RANGE>"
      # The IP address range for Private Service Connect (e.g., "10.1.0.0/16")
    }
  ...
  dataflow_params = {
    ...
    template_params = {
      ...
      # Prefix for shadow tables (e.g., "shadow_")
      create_shadow_tables                = "<TRUE/FALSE>"
      # Whether to create shadow tables in Spanner
      rfc_start_date_time                 = "<YOUR_RFC_START_DATETIME>"
      # RFC 3339 timestamp for the start of replication (optional)
      ...
      dlq_retry_minutes                   = "<YOUR_DLQ_RETRY_MINUTES>"
      # Retry interval for dead-letter queue messages (in minutes)
      ...
      # Datastream API root URL (typically "https://datastream.googleapis.com/v1")
      datastream_source_type              = "<YOUR_DATASTREAM_SOURCE_TYPE>"
      # Datastream source type (e.g., "mysql")
      round_json_decimals                 = "<TRUE/FALSE>"# Whether to round JSON decimal values in Dataflow
      directory_watch_duration_in_minutes = "<YOUR_WATCH_DURATION>"
      # Directory watch duration (in minutes) for Dataflow
      spanner_priority                    = "<YOUR_SPANNER_PRIORITY>"
      # Spanner priority ("HIGH", "MEDIUM", or "LOW")
      local_session_file_path             = "<YOUR_SESSION_FILE_PATH>"
      # Path to local session file (optional)
      transformation_jar_path             = "<YOUR_CUSTOM_TRANSFORMATION_JAR_PATH>"
      # GCS path to the custom transformation JAR(Optional)
      transformation_custom_parameters    = "<YOUR_CUSTOM_PARAMETERS_FOR_JAR>"
      # Custom parameters used by the transformation JAR(Optional)
      transformation_class_name           = "<YOUR_TRANSFORMATION_CLASS_NAME>"
      # Fully Classified Class Name(Optional)
      filtered_events_directory           = "<YOUR_GCS_PATH_FOR_FILTERED_EVENTS>"
      # GCS path to store the filtered events(Optional)
    }
    ...
    runner_params = {
      ...
      launcher_machine_type        = "<YOUR_LAUNCHER_MACHINE_TYPE>"
      # Machine type for the launcher VM (e.g., "n1-standard-1")
      machine_type                 = "<YOUR_MACHINE_TYPE>"
      # Machine type for worker VMs (e.g., "n2-standard-2")
      ...
      skip_wait_on_job_termination = "<TRUE/FALSE>"
      # Whether to skip waiting for job termination on deletion
      ...
      on_delete                    = "<YOUR_ON_DELETE_ACTION>"
      # Action on Dataflow job deletion ("cancel" or "drain")
      ip_configuration             = "<YOUR_IP_CONFIGURATION>"
      # IP configuration for Dataflow workers ("WORKER_IP_PRIVATE" or "WORKER_IP_PUBLIC")
    }
  }
}
...
shard_list = [
  {
    ...
    datastream_params = {
      ...
      # Datastream target connection profile ID (GCS)
      ...
      # Datastream stream ID (will be prefixed with common_params.datastream_params.stream_prefix_path)
    }
    ...
    dataflow_params = {
      template_params = {
        ...
        dlq_gcs_pub_sub_subscription      = "<YOUR_DLQ_SUBSCRIPTION>"
        # Pub/Sub subscription for the dead-letter queue (optional)
      }
