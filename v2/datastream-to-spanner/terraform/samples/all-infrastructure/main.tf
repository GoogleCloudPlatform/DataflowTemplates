...
resource "google_datastream_connection_profile" "source_mysql" {
  ...
  display_name          = "${var.datastream_params.source_connection_profile_id}-${random_pet.migration_suffix.id}"
  location              = var.common_params.region
  connection_profile_id = "${var.datastream_params.source_connection_profile_id}-${random_pet.migration_suffix.id}"

  mysql_profile {
    hostname = var.datastream_params.mysql_host
    username = var.datastream_params.mysql_username
    password = var.datastream_params.mysql_password
    port     = var.datastream_params.mysql_port
  }
}
...
resource "google_storage_bucket" "datastream_bucket" {
  ...
  name                        = "${var.datastream_params.gcs_bucket_name}-${random_pet.migration_suffix.id}"
  location                    = var.common_params.region
...
resource "google_datastream_connection_profile" "target_gcs" {
  ...
  display_name          = "${var.datastream_params.target_connection_profile_id}-${random_pet.migration_suffix.id}"
  location              = var.common_params.region
  connection_profile_id = "${var.datastream_params.target_connection_profile_id}-${random_pet.migration_suffix.id}"
...
resource "google_datastream_stream" "mysql_to_gcs" {
  ...
  source_config {
    source_connection_profile = google_datastream_connection_profile.source_mysql.id
