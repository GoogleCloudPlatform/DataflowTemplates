...
variable "datastream_params" {
  ...
  type        = object({
    ...
    gcs_bucket_name               = optional(string, "live-migration-bucket")
    gcs_root_path                 = optional(string, "/")
    pubsub_topic_name             = optional(string, "live-migration-topic")
  ...
  type        = object({
    ...
    runner_params = object({
      additional_experiments = optional(set(string), [
        "enable_google_cloud_profiler", "disable_runner_v2"
      ])
